...
Select your <VPIcon icon="iconfont icon-xcode"/>`Main.storyboard` file to open Interface Builder, then embed the view controller inside a navigation controller.
...
That's it for the storyboard, so bring up <VPIcon icon="fa-brands fa-swift"/>`ViewController.swift` and let's start coding…
