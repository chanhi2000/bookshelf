...
Open <VPIcon icon="fa-brands fa-swift"/>`DetailViewController.swift`, then add these two new methods directly below the end of the `viewDidLoad()` method:
...
First, add this to `viewDidLoad()` in <VPIcon icon="fa-brands fa-swift"/>`ViewController.swift`:
...
In this app we want “Storm Viewer” to appear big, but the detail screen to look normal. To make that happen we need to add a line of code to `viewDidLoad()` in <VPIcon icon="fa-brands fa-swift"/>`DetailViewController.swift`:
