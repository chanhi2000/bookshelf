...
That's all the layout done, so we're finished with Interface Builder and you can open up <VPIcon icon="fa-brands fa-swift"/>`GameViewController.swift`.
...
To make your code compile, you need to add a `launch()` method to <VPIcon icon="fa-brands fa-swift"/>`GameScene.swift`. It doesn't need to be the real thing, but it does need to accept parameters for angle and velocity. Give it this code for now:
