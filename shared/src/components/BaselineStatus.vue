  ...
  methods: {
    async fetchData() {
      try {
        ...
        this.baselineDate = this.getBaselineDate(data) ?? undefined;
      } catch (error) {
      ...
      this.description = this.getDescriptionDate(this.baseline, this.baselineDate);
    ...
    getAriaLabel() {
      const badge = this.baseline === 'newly' 
        ...
        : '';
      const year = (this.baseline === 'newly' && this.baselineDate)
        ? this.baselineDate.split(' ')[1]
        : '';
