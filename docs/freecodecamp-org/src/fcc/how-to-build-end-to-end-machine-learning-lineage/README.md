...
In this project, I’ll integrate an ML lineage into [<FontIcon icon="fas fa-globe"/>this price prediction system built on AWS Lambda architecture](https://levelup.gitconnected.com/building-a-dynamic-pricing-system-with-a-multi-layered-neural-network-c2a4c70bfcec) using DVC, an open-source version control system for ML applications.

The below diagram illustrates the system architecture and the ML lineage we’ll integrate:

![Figure A. A comprehensive ML lineage for an ML application on serverless Lambda (Created by [<FontIcon icon="fas fa-globe"/>Kuriko IWAI](https://kuriko-iwai.vercel.app/))](https://cdn.hashnode.com/res/hashnode/image/upload/v1759825040233/5027e5dd-a2fc-4d35-b7a3-4d9184f5f179.png)
...
2. Define the lineage stages with the DVC script <FontIcon icon="iconfont icon-yaml"/>`dvc.yaml` and corresponding Python script
...
This command automatically creates a <FontIcon icon="fas fa-folder-open"/>`.dvc` directory at the root of the project folder:
...
The process involves caching the original data in the local <FontIcon icon="fas fa-folder-open"/>`.dvc/cache` directory, creating a small <FontIcon icon="fas fa-folder-open"/>`.dvc` metadata file which contains an MD5 hash and a link to the original data file path, pushing *only* the small metadata files to Git, and pushing the original data to the DVC remote.
...
We’ll create the <FontIcon icon="iconfont icon-yaml"/>`dvc.yaml` file at the root of the project directory and add the `etl_pipeline` stage:
...
The <FontIcon icon="iconfont icon-yaml"/>`dvc.yaml` file defines a sequence of steps (stages) using sections like:
...
Next, let’s add Python scripts, ensuring the data is stored using the file paths specified in the `outs` section of the <FontIcon icon="iconfont icon-yaml"/>`dvc.yaml` file:
...
After [<FontIcon icon="fas fa-globe"/>generating an API token from the EventlyAI workplace,](https://docs.evidentlyai.com/quickstart_ml#1-1-set-up-evidently-cloud) we’ll add a Python script to detect data drift and store the results in the `metrics` variable:
...
- <FontIcon icon="fas fa-folder-open"/>`reports/`<FontIcon icon="fa-brands fa-html5"/>`data_drift_report.html`: The data drift report in a HTML file.
- <FontIcon icon="fas fa-folder-open"/>`metrics/`<FontIcon icon="iconfont icon-json"/>`data_drift.json`: The data drift metics in a JSON file including drift results along with feature columns and a timestamp:
...
Lastly, DVC adds the `preprocess_status` , `x_train_processed_path`, and `preprocessor_path` to the data summary metrics file <FontIcon icon="iconfont icon-json"/>`data.json` created in Step 2 to track the end-to-end process of Steps 2 and 3:
...
- `models/production/`<FontIcon icon="fas fa-file-lines"/>`dfn_best.pth`: Includes model artifacts and checkpoint like the optimal hyperparameter set.
- `metrics`<FontIcon icon="iconfont icon-json"/>`/dfn_val.json`: Contains tuning results, model version, timestamp, and validation results for MSE, MAE, and RMSLE:
...
- <FontIcon icon="fas fa-folder-open"/>`data/`<FontIcon icon="fas fa-file-lines"/>`dfn_inference_result_${params_stockcode}.parquet`: Stores prediction results, labeled targets, and any columns with sensitive features like gender, age, income, and more. I’ll use this file for the fairness test in the last stage.
- <FontIcon icon="fas fa-folder-open"/>`metrics/`<FontIcon icon="iconfont icon-json"/>`dfn_inf.json`: Stores evaluation metrics and tuning results:
...
- <FontIcon icon="fas fa-folder-open"/>`reports/`<FontIcon icon="iconfont icon-json"/>`dfn_shap_summary.json`: Contains the data points necessary to draw the beeswarm/bar plots.
- <FontIcon icon="fas fa-folder-open"/>`reports/`<FontIcon icon="fas fa-file-lines"/>`dfn_raw_shap_values.parquet`: Stores raw SHAP values.
...
The command will automatically create the <FontIcon icon="iconfont icon-yaml"/>`dvc.lock` file at the root of the project directory:
...
The <FontIcon icon="iconfont icon-yaml"/>`dvc.lock` file must be published in Git to make sure DVC will load the latest files:
...
The final step is to deploy the entire application as a containerized Lambda by configuring the <FontIcon icon="fa-brands fa-docker"/>`Dockerfile` and the Flask application scripts.
...
You can learn more here: [Integrating the infrastructure CI/CD pipeline to an ML application (<FontIcon icon="fa-brands fa-medium"/>`towards-artificial-intelligence`)](https://medium.com/towards-artificial-intelligence/integrating-ci-cd-pipelines-to-machine-learning-applications-f5657c7fa164)
...
All code is available in [my GitHub repository (<FontIcon icon="iconfont icon-github"/>`krik8235/ml-sales-prediction`)](https://github.com/krik8235/ml-sales-prediction).
...
The mock app is available [<FontIcon icon="fas fa-globe"/>here](https://kuriko-iwai.vercel.app/online-commerce-intelligence-hub).
...
You can check out my [<FontIcon icon="fas fa-globe"/>Portfolio](https://kuriko-iwai.vercel.app/) / [Github (<FontIcon icon="iconfont icon-github"/>`krik8235`)](https://github.com/krik8235).
