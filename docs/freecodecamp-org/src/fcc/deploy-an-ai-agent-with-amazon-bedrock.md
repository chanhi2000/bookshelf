...
First, install the AWS CLI if you do not already have it. On Linux or macOS: [<FontIcon icon="fa-brands fa-aws"/>AWS CLI setup guide](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html).

Next, [<FontIcon icon="fa-brands fa-aws"/>configure](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html) a profile with AWS SSO:
...
- `python -m venv .venv` → creates a virtual environment named <FontIcon icon="fas fa-folder-open"/>`.venv` in your project folder.
...
List the dependencies your project needs by creating a file named <FontIcon icon="fas fa-file-lines"/>`requirements.txt` in the project root:
...
Create a file called <FontIcon icon="fa-brands fa-python"/>`my_agent.py` and add the following code:
...
This creates a configuration file called <FontIcon icon="iconfont icon-yaml"/>`bedrock_agentcore.yaml`.
...
You can call your agent programmatically using Boto3. Create a file called <FontIcon icon="fa-brands fa-python"/>`invoke_agent.py`:
