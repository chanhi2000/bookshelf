...
head:
  - - meta:
    ...
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/fcc/empire-state-building-run-up-analysis-with-python/how-to-visualize-the-results.html
date: 2024-12-03
isOriginal: false
author:
...
I wanted to use the terminal as much as possible to visualize my findings, and to keep requirements to a minimum. I decided to use the [<VPIcon icon="fas fa-globe"/>Textual](https://textual.textualize.io/) framework to accomplish that.
...
The [<VPIcon icon="fas fa-globe"/>Textual project](https://textual.textualize.io/) has a nice tutorial that [<VPIcon icon="fas fa-globe"/>you can read](https://textual.textualize.io/tutorial/) to get up to speed.

Let's see some code. One of the applications is called `esru_outlier`. TUI code lives on the [apps (<VPIcon icon="iconfont icon-github"/>`josevnz/tutorials`)](https://github.com/josevnz/tutorials/blob/main/docs/EmpireStateRunUp/empirestaterunup/apps.py) module that shows several tables together with the outliers we found before, using the z-score.
...
We can customize the appearance using CSS (yes, like a web application). It looks a lot like a web application's [<VPIcon icon="fa-brands fa-wikipedia-w"/>CSS](https://en.wikipedia.org/wiki/CSS) (but it's not exactly the same). For example to add style to a button, here's the code:
...
As you can see, Textual is a pretty powerful framework. It reminds me a lot of [<VPIcon icon="fa-brands fa-wikipedia-w"/>Java Swing](https://en.wikipedia.org/wiki/Swing_(Java)), but without the extra complexity.
...
I wanted to use some charts to display the data, and I made them with [<VPIcon icon="iconfont icon-matplotlib"/>matplotlib](https://matplotlib.org/). The code to generate an age plot box, that shows how old the participating runners were, is very straightforward.
...
This is invaluable, and something that many times requires an external toolset to validate (for example in Java you have the class [<VPIcon icon="fa-brands fa-java"/>Robot](https://docs.oracle.com/javase/8/docs/api/java/awt/Robot.html)).
