...
This is going to be a complete hands-on tutorial. So get your favourite code editor ready, and let’s get started. This article is also available as a video tutorial as part of the [<FontIcon icon="fa-brands fa-youtube"/>15 Days of React Design Patterns](https://youtube.com/playlist?list=PLIJrr73KDmRyQVT__uFZvaVfWPdfyMFHC) initiative. Please check it out.
...
Note: you can find all the source code used in this tutorial on the [tapaScript GitHub (<FontIcon icon="iconfont icon-github"/>`tapascript/15-days-of-react-design-patterns`)](https://github.com/tapascript/15-days-of-react-design-patterns/tree/main/day-03/compound-components-patterns). Feel free to follow along with it side by side.
...
If you get an output with the installed Node.js version, you are all set. Otherwise, just download and install Node.js from [<FontIcon icon="fa-brands fa-node"/>here](https://nodejs.org/en/download).
...
Let’s get started by creating a Modal component. Start by creating a directory called <FontIcon icon="fas fa-folder-open"/>`messy` under the <FontIcon icon="fas fa-folder-open"/>`src/` directory. Now, create a file called `Modal.jsx` under `src/messy/` with the following code snippet:
...
Next, open the <FontIcon icon="fa-brands fa-react"/>`App.jsx` file and replace the existing code with the following code snippet:
...
The modal tries to do multiple things. It handles both layout and content. This violates the separation of concerns principle that we learn from other design patterns, such as the [<FontIcon icon="fa-brands fa-youtube"/>Container-Presenter Pattern](https://youtu.be/1UHbhikwg-s).
...
Open the <FontIcon icon="fa-brands fa-react"/>`App.jsx` file and replace the content of it with the following code snippet:
...
Create a folder called <FontIcon icon="fas fa-folder-open"/>`accordion` under <FontIcon icon="fas fa-folder-open"/>`src/with-pattern` folder. Now, create a file called <FontIcon icon="fa-brands fa-react"/>`Accordion.jsx` inside the <FontIcon icon="fas fa-folder-open"/>`src/with-pattern/accordion` with the following code snippet:
...
Great, let’s now use the Accordion component. Create a new file called <FontIcon icon="fa-brands fa-react"/>`AccordionDemo.jsx` under the folder <FontIcon icon="fas fa-folder-open"/>`src/with-pattern/accordion/`<FontIcon icon="fa-brands fa-react"/>`AccordionDemo.jsx` with the following code snippet:
...
I have some great news for you! After the *40 days of the JavaScript* initiative, I have now started a brand new initiative called [<FontIcon icon="fa-brands fa-youtube"/>15 Days of React Design Patterns](https://youtube.com/playlist?list=PLIJrr73KDmRyQVT__uFZvaVfWPdfyMFHC).
...
- Subscribe to my fortnightly newsletter, [<FontIcon icon="fas fa-globe"/>The Commit Log](https://tapascript.substack.com/subscribe?utm_medium=fcc).
...
- Join my [<FontIcon icon="fa-brands fa-discord"/>Discord Server](https://discord.gg/zHHXx4vc2H), and let’s learn together.
