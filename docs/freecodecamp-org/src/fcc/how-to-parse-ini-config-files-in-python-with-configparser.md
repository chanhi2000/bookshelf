...
INI files, short for initialization files, with their simple section-based format, are both easy to read and parse. Python's built-in [<FontIcon icon="fa-brands fa-python"/>configparser module](https://docs.python.org/3/library/configparser.html) makes working with these files straightforward and powerful.

This tutorial will teach you how to read and parse such <FontIcon icon="fas fa-file-lines"/>`.ini` config files using the `configparser` module.
...
Create a file named <FontIcon icon="iconfont icon-toml"/>`app.ini`:
...
Create the <FontIcon icon="iconfont icon-toml"/>`app.ini` file and run the above code. You should see the following output:
...
- Keep secrets separate. Don't commit INI files with passwords to version control. Use <FontIcon icon="iconfont icon-toml"/>`.ini.example` files with dummy values as templates.
