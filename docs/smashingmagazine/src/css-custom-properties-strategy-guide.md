...
description: "Article(s) > A Strategy Guide To CSS Custom Properties"
...
head:
  - - meta:
    - property: og:title
      content: "Article(s) > A Strategy Guide To CSS Custom Properties"
    - property: og:description
      content: "A Strategy Guide To CSS Custom Properties"
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/smashingmagazine.com/css-custom-properties-strategy-guide.html
prev: /programming/css/articles/README.md
date: 2018-05-14
isOriginal: false
author:
  - name: Michael Riethmuller
    url: https://smashingmagazine.com/author/michaelriethmuller/
cover: https://archive.smashing.media/assets/344dbf88-fdf9-42bb-adb4-46f01eedd629/a0d1f2dd-b44b-45e5-ad01-29418cbe0fd7/google-keep-app.png
---
...
<SiteInfo
  ...
  desc="Dynamic properties provide opportunities for new creative ideas, but also the potential to add complexity to CSS. Custom properties have a huge potential to change how we write and structure CSS and to a lesser extent, how we use JavaScript to interact with UI components. To get the most out of them, we might need a strategy for how we write and structure CSS with custom properties. In this article, Michael Riethmuller will show you how."
  url="https://smashingmagazine.com/2018/05/css-custom-properties-strategy-guide/"
  logo="https://smashingmagazine.com/images/favicon/favicon.svg"
  preview="https://archive.smashing.media/assets/344dbf88-fdf9-42bb-adb4-46f01eedd629/a0d1f2dd-b44b-45e5-ad01-29418cbe0fd7/google-keep-app.png"/>
...
Custom properties have a huge potential to change how we write and structure CSS and to a lesser extent, how we use JavaScript to interact with UI components. I’m not going to focus on the syntax and how they work (for that I recommend you read “[**It’s Time To Start Using Custom Properties**](/smashingmagazine.com/start-using-css-custom-properties.md)”). Instead, I want to take a deeper look at strategies for getting the most out of CSS Custom Properties.
...
![buttons](https://res.cloudinary.com/indysigner/image/fetch/f_auto,q_80/w_400/https://archive.smashing.media/assets/344dbf88-fdf9-42bb-adb4-46f01eedd629/b8f668c2-7b33-4fe9-a410-210138e711d6/buttons-small-medium-large.png)
...
Did you know that `$var` is valid value for a custom property? Recent versions of Sass recognize this, and therefore we need to interpolate variables assigned to custom properties, like this: `#{$var}`. This tells Sass you want to output the value of the variable, rather than just `$var` in the stylesheet. This is only needed for situations like custom properties, where variable names can also be a valid CSS.
...
<!-- TODO: /freecodecamp.org/dont-do-it-at-runtime-do-it-at-design-time.md -->
...
![Google Keep App](https://res.cloudinary.com/indysigner/image/fetch/f_auto,q_80/w_400/https://archive.smashing.media/assets/344dbf88-fdf9-42bb-adb4-46f01eedd629/a0d1f2dd-b44b-45e5-ad01-29418cbe0fd7/google-keep-app.png)
...
You can see how this works with this [example using React (<VPIcon icon="fa-brands fa-codepen" />`MadeByMike`)](https://codepen.io/MadeByMike/pen/00ae89e134c7cb6914b417cb5c494f7d?editors=0110). These user preferences could be saved in local storage or, perhaps in the case of a larger application, in a database.
...
It would be useful to have some of these features in browsers. [<VPIcon icon="iconfont icon-w3c"/>They are coming](https://w3.org/TR/css-color-4/#funcdef-color-mod), but until we have native color modification functions in CSS, custom properties could fill some of that gap.
...
We *could* even abstract away more of the calculations and create something like [color modification functions in CSS using custom properties (<VPIcon icon="fa-brands fa-codepen" />`MadeByMike`)](https://codepen.io/MadeByMike/pen/YLQWeb). This example is likely too complex for most practical cases of theming, but it demonstrates the full power of dynamic custom properties.
...
There are many ways you can use postCSS. I use a [<VPIcon icon="fas fa-globe"/>`gulp`](https://gulpjs.com/) process to compile separate stylesheets for newer and older browsers. A simplified version of my `gulp` task looks like this:
...
This results in two CSS files: a regular one with custom properties (<VPIcon icon="fa-brands fa-css3-alt"/>`styles.css`) and one for older browsers (<VPIcon icon="fa-brands fa-css3-alt"/>`styles.no-vars.css`). I want IE10 and 11 to be served <VPIcon icon="fa-brands fa-css3-alt"/>`styles.no-vars.css` and other browsers to get the regular CSS file.

Normally, I’d advocate using feature queries but [<VPIcon icon="iconfont icon-caniuse"/>IE11 doesn’t support feature queries](https://caniuse.com) and we’ve used custom properties so extensively that serving a different stylesheet makes sense in this case.

Intelligently serving a different stylesheet and avoiding a flash of unstyled content is not a simple task. If you don’t need the dynamic features of custom properties, you could consider serving all browser <VPIcon icon="fa-brands fa-css3-alt"/>`styles.no-vars.css` and using custom properties simply as a development tool.

If you want to take full advantage of all the dynamic features of custom properties, I suggest using a [**critical CSS technique**](/smashingmagazine.com/understanding-critical-css,md). Following these techniques, the main stylesheet is loaded asynchronously while the critical CSS is rendered inline. Your page header might look something like this:
...
- “[It’s Time To Start Using Custom Properties](https://smashingmagazine.com/2017/04/start-using-css-custom-properties/),” Serg Hospodarets  
...
- [A Few Ways CSS Is Easier To Write In 2023](https://smashingmagazine.com/2023/11/few-ways-css-easier-write-2023/)
- [The Path To Awesome CSS Easing With The linear() Function](https://smashingmagazine.com/2023/09/path-css-easing-linear-function/)
- [Gradients, Blend Modes, And A Really Cool Hover Effect](https://smashingmagazine.com/2023/09/gradients-blend-modes-hover-effect/)
- [Revealing Images With CSS Mask Animations](https://smashingmagazine.com/2023/09/revealing-images-css-mask-animations/)
...
```component VPCard
{
  "title": "A Strategy Guide To CSS Custom Properties",
  "desc": "Dynamic properties provide opportunities for new creative ideas, but also the potential to add complexity to CSS. Custom properties have a huge potential to change how we write and structure CSS and to a lesser extent, how we use JavaScript to interact with UI components. To get the most out of them, we might need a strategy for how we write and structure CSS with custom properties. In this article, Michael Riethmuller will show you how.",
  "link": "https://chanhi2000.github.io/bookshelf/smashingmagazine.com/css-custom-properties-strategy-guide.html",
  "logo": "https://smashingmagazine.com/images/favicon/favicon.svg",
