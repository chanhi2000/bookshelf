...
In contrast, a **free variable** is not bound and may be referenced in the body of the expression. A free variable can be a constant or a variable defined in the enclosing [**scope**](/realpython.com/python-namespace.md) of the function.
...
At this point, you may wonder what fundamentally distinguishes a lambda function bound to a variable from a regular function with a single `return` line: under the surface, almost nothing. Let’s verify how Python sees a function built with a single [**return statement**](/realpython.com/python-return-statement/README.md) versus a function constructed as an expression (`lambda`).
