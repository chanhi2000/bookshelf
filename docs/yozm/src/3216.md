...
제미나이 CLI에 입력하는 프롬프트는 자연어 입력도 가능하고, 터미널 명령어도 실행 가능합니다. 먼저 터미널 명령어의 예시인 `ls` 를 입력 후 엔터를 클릭하면 현재 위치한 디렉터리의 내용을 확인하고 이를 설명합니다. 저는 <VPIcon icon="fas fa-folder-open"/>`~/Documents/gemini`를 새롭게 만들어 해당 디렉터리에서 작업했습니다.
