...
If you haven't already downloaded the assets for this project from [GitHub (<VPIcon icon="iconfont icon-github"/>`twostraws/HackingWithSwift`)](https://github.com/twostraws/HackingWithSwift), please do so now. In the project5-files folder you'll find the file start.txt - please drag that into your Xcode project, making sure that "Copy items if needed" is checked.
...
So, open <VPIcon icon="fa-brands fa-swift"/>`ViewController.swift` and add these two properties:
...
Now press <kbd>⌘</kbd>+<kbd>R</kbd> to run the app, and you ought to see an eight-letter word at the top, ready for play to begin.
