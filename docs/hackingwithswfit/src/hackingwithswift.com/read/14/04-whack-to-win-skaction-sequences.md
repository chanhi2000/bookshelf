...
head:
  - - meta:
    ...
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/hackingwithswift.com/read/14/04-whack-to-win-skaction-sequences.html
isOriginal: false
---
...
Now, looking at our `touchesBegan()` method in <VPIcon icon="fa-brands fa-swift"/>`GameScene.swift`, you should see we can actually move the code around a little to remove duplication. Specifically, checking `isVisible` and `isHit` doesn’t need to be done twice, and neither does calling `whackSlot.hit()` - a better idea is to move those lines outside of their conditions, like this:
