...
So, press <kbd>⌘</kbd>+<kbd>N</kbd> to make a new SwiftUI view now, and call it `EditDestinationView`. When Xcode opens it for editing, please add `import SwiftData` near the top, so we get access to all the SwiftData API.
