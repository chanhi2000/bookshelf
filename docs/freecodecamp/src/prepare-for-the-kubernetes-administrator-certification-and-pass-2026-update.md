---
lang: en-US
title: "Prepare for the Kubernetes Administrator Certification and Pass [2026 update]"
description: "Article(s) > Prepare for the Kubernetes Administrator Certification and Pass [2026 update]"
...
head:
  - - meta:
    - property: og:title
      content: "Article(s) > Prepare for the Kubernetes Administrator Certification and Pass [2026 update]"
    - property: og:description
      content: "Prepare for the Kubernetes Administrator Certification and Pass [2026 update]"
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/freecodecamp.org/prepare-for-the-kubernetes-administrator-certification-and-pass-2026-update.html
...
cover: https://cdn.hashnode.com/res/hashnode/image/upload/v1770737611970/40c85fc5-30e0-450c-8473-b05124831718.jpeg
---
...
<SiteInfo
  name="Prepare for the Kubernetes Administrator Certification and Pass [2026 update]"
  ...
  url="https://freecodecamp.org/news/prepare-for-the-kubernetes-administrator-certification-and-pass-2026-update"
  logo="https://cdn.freecodecamp.org/universal/favicons/favicon.ico"
  preview="https://cdn.hashnode.com/res/hashnode/image/upload/v1770737611970/40c85fc5-30e0-450c-8473-b05124831718.jpeg"/>
...
You can watch the course on [<VPIcon icon="fa-brands fa-youtube"/>the freeCodeCamp.org YouTube channel](https://youtu.be/l57xKN6OBhY) (2-hour watch).

This course was made possible by a grant from Linux Foundation. Use code FREECODECAMP to get 30% off training, certifications, and bundles from Linux Foundation.

<VidStack src="youtube/l57xKN6OBhY" />
...
# !! IMPORTANT: Manually edit /etc/kubernetes/manifests/etcd.yaml to point to 
#               the new data-dir /var/lib/etcd-restored !!
...
Save the HA-specific join command and the `--certificate-key`.
...
```sh
#
# 1. Create a Namespace and ServiceAccount:1. Create a Namespace and ServiceAccount:
...
1. Create base manifest (<VPIcon icon="fas fa-folder-open"/>`my-app/base/`<VPIcon icon="iconfont icon-yaml"/>`deployment.yaml`)
...
```sh
# Apply:
kubectl apply -f pod-config.yaml` **Verify: `kubectl logs config-demo-pod
```
...
```sh
# Apply:
kubectl apply -f pod-volume.yaml` **Verify: `kubectl logs volume-demo-pod
```
...
# (requires `toleration-pod.yaml` manifest not provided, create a dummy pod for the taint)
...
# (requires `clusterip-service.yaml` manifest not provided, use an imperative command)
...
# (requires `nodeport-service.yaml` manifest not provided, use an imperative command)
...
# (requires `ingress.yaml` manifest not provided, use the provided structure to create the file)
# Apply ingress.yaml
...
```sh
kubectl run tmp-shell --rm -it --labels=access=true \
--image=busybox -- /bin/sh -c "wget -O- web-server"
```
...
2. Add a new server block inside the `Corefile` data structure
...
```sh
#
# 1. Get detailed information on a resource
# (the most critical debugging command):
...
```component VPCard
{
  "title": "Prepare for the Kubernetes Administrator Certification and Pass [2026 update]",
  "desc": "We just posted a course on the freeCodeCamp.org YouTube channel to help prepare you for the Certified Kubernetes Administrator Certification. This course is designed to provide a deep, practical understanding of Kubernetes administration, from founda...",
  "link": "https://chanhi2000.github.io/bookshelf/freecodecamp.org/prepare-for-the-kubernetes-administrator-certification-and-pass-2026-update.html",
  "logo": "https://cdn.freecodecamp.org/universal/favicons/favicon.ico",
