...
Following this process, we’ll arrive at a DDL script [like this (<VPIcon icon="iconfont icon-github"/>`cardstdani`)](https://gist.github.com/cardstdani/1247573e1ef2f6ea9ab99b82c5761ad6) (it’s quite long, so I’ve left it in the gist).
...
To figure out how we need to order both the DROP and `CREATE TABLE` statements, there are [algorithms (<VPIcon icon="fa-brands fa-medium"/>`@tharinduimalka915`)](https://medium.com/@tharinduimalka915/how-kahns-algorithm-helped-me-solve-database-schema-dependencies-2b7e54142fd5) like [<VPIcon icon="fa-brands fa-wikipedia-w"/>topological sorting](https://en.wikipedia.org/wiki/Topological_sorting) that we can apply to the relational diagram. This way, we treat the database schema as a **directed graph** made up of **nodes (tables)** and **directed edges (foreign keys)**. With this algorithm, for example, we can progressively remove minimal or maximal nodes from the graph, creating or deleting the table they represent. But, this is not the only [<VPIcon icon="fas fa-globe"/>method](https://softwareengineering.stackexchange.com/questions/359107/resolving-foreign-keys-breaking-cycles-to-enable-a-topological-sort) available.
...
We can also assign an owner user, who will have all the [<VPIcon icon="iconfont icon-postgresql"/>privileges](https://postgresql.org/docs/current/ddl-priv.html) over that element. By default, we can make the owner user [<VPIcon icon="fa-brands fa-stack-overflow"/>postgres](https://stackoverflow.com/questions/50883645/is-postgres-a-default-and-special-user-of-postgresql), but we can change it later with a statement like the following:
