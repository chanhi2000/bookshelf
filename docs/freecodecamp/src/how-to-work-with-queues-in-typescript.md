...
If any of the tests fail, use the final example from <VPIcon icon="fas fa-folder-open"/>`src/examples/`<VPIcon icon="iconfont icon-typescript"/>`02-simple-queue.ts` to debug the issue, and then run the tests again.
