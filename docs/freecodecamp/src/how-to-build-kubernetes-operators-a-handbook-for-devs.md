---
lang: en-US
title: "How to Build Kubernetes Operators: A Handbook for Devs"
description: "Article(s) > How to Build Kubernetes Operators: A Handbook for Devs"
...
head:
  - - meta:
    - property: og:title
      content: "Article(s) > How to Build Kubernetes Operators: A Handbook for Devs"
    - property: og:description
      content: "How to Build Kubernetes Operators: A Handbook for Devs"
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/freecodecamp.org/how-to-build-kubernetes-operators-a-handbook-for-devs.html
...
cover: https://cdn.hashnode.com/uploads/covers/5fc16e412cae9c5b190b6cdd/21c81312-eb74-40f3-823a-3831945a3f58.png
---
...
<SiteInfo
  name="How to Build Kubernetes Operators: A Handbook for Devs"
  desc="Kubernetes ships with controllers that manage a fixed set of built-in resources: Deployments, Services, Nodes, and so on. An operator extends the same pattern to resources Kubernetes doesn't know abou"
  url="https://freecodecamp.org/news/how-to-build-kubernetes-operators-a-handbook-for-devs"
  logo="https://cdn.freecodecamp.org/universal/favicons/favicon.ico"
  preview="https://cdn.hashnode.com/uploads/covers/5fc16e412cae9c5b190b6cdd/21c81312-eb74-40f3-823a-3831945a3f58.png"/>
...
![reconciliation loop illustration](https://raw.githubusercontent.com/karanpratapsingh/portfolio/refs/heads/master/public/static/blogs/kubernetes-operators/reconcile-loop.png)
<!-- TODO: mermaid화 -->
...
A [<VPIcon icon="iconfont icon-helm"/>Helm chart](https://helm.sh/docs/topics/charts/) renders a set of values into YAML and applies it once. It has no way to keep watching afterward. if a resource it created is deleted or drifts, Helm has no idea until we run `helm upgrade` again by hand.
...
- **Spec** is desired state. Whoever creates or edits the resource writes it, and the controller only ever reads it.
...
We'll see exactly what that registration looks like when we build one for real in Part 3. ### Watching for Change
...
![informer, workqueue, and reconciler pipeline](https://raw.githubusercontent.com/karanpratapsingh/portfolio/refs/heads/master/public/static/blogs/kubernetes-operators/informer-pipeline.png)
<!-- TODO: mermaid화 -->
...
![VMOperator architecture](https://raw.githubusercontent.com/karanpratapsingh/portfolio/refs/heads/master/public/static/blogs/kubernetes-operators/vmoperator-architecture.png)

### Setup

We'll need a local cluster, and [<VPIcon icon="fas fa-globe"/>kind](https://kind.sigs.k8s.io/) is the easiest way to get one:

```sh
kind create cluster --name vmoperator
```

Beyond that, we'll be using [<VPIcon icon="fa-brands fa-golang"/>Go](https://go.dev/doc/install) in this part for the operator, `kubectl` pointed at the new cluster, and Python with Flask for the mock provider below.
...
Every `VirtualMachine` so far talks to one hardcoded provider endpoint. Real deployments need that to be configurable, and it's rarely a one-off: fifty `VirtualMachine`s in the same AWS account share the same endpoint and credentials, and a hundred more might live in Azure instead.

We could put an `endpoint` field directly on `VirtualMachineSpec`, but rotating a credential or fixing a typo would then mean editing every `VirtualMachine` that uses it, one at a time. Pulling that into its own object lets many `VirtualMachine`s reference it by name instead, so a single edit propagates to all of them.
...
![cross-resource reconciliation fan-out](https://raw.githubusercontent.com/karanpratapsingh/portfolio/refs/heads/master/public/static/blogs/kubernetes-operators/cross-resource-fanout.png)
...
VMOperator manages a resource entirely outside the cluster through a hand-rolled HTTP client, but it's not a novel pattern. [<VPIcon icon="fas fa-globe"/>Crossplane](https://crossplane.io/), [<VPIcon icon="fas fa-globe"/>AWS Controllers for Kubernetes](https://aws-controllers-k8s.github.io/community/), [<VPIcon icon="fas fa-globe"/>Cluster API](https://cluster-api.sigs.k8s.io/), and cert-manager all reconcile external or non-Kubernetes state through CRDs the same way. These resources are worth reading once this pattern feels familiar.

:::

::: note Another note

We're keeping VMOperator's scope narrow on purpose. Resizing a running VM, stopping and restarting one, taking snapshots, and supporting more than one real provider behind `ProviderConfig` are all natural extensions of what's here, and a reasonable next step once the core loop feels solid.
...
A `Deployment` needs an image instead, so the operator gets a multi-stage <VPIcon icon="fa-brands fa-docker"/>`Dockerfile`: one stage to compile it, and a second, much smaller image to actually run it:
...
A tighter version scopes to a single namespace with `Role`/`RoleBinding` instead of `ClusterRole`/`ClusterRoleBinding` wherever VMOperator is only expected to run in one, and it drops verbs we never call. We never `list` or `watch` arbitrary `Secret`s outside our own, only the ones we create. This is also the RBAC the manifests applied in the previous section were referring to.
...
<SiteInfo
  ...
  desc="Go client for Kubernetes.."
  url="https://github.com/kubernetes/client-go/"
  logo="https://github.githubassets.com/favicons/favicon-dark.svg"
  preview="https://opengraph.githubassets.com/7fe95295e1d2c9124012c091179bfba5b38ed83d7ce4b59e8221bc8cce238062/kubernetes/client-go"/>

<SiteInfo
  name="kubernetes-sigs/controller-runtime"
  desc="Repo for the controller-runtime subproject of kubebuilder (sig-apimachinery)"
  url="https://github.com/kubernetes-sigs/controller-runtime/"
  logo="https://github.githubassets.com/favicons/favicon-dark.svg"
  preview="https://opengraph.githubassets.com/2b2744b670fc2157704784e34b6faa7e0ba393da5afe4898f49c05de2179a3c1/kubernetes-sigs/controller-runtime"/>

<SiteInfo
  name="Docker Build"
  desc="Get an overview of Docker Build to package and bundle your code and ship it anywhere"
...
```component VPCard
{
  "title": "How to Build Kubernetes Operators: A Handbook for Devs",
  "desc": "Kubernetes ships with controllers that manage a fixed set of built-in resources: Deployments, Services, Nodes, and so on. An operator extends the same pattern to resources Kubernetes doesn't know abou",
  "link": "https://chanhi2000.github.io/bookshelf/freecodecamp.org/how-to-build-kubernetes-operators-a-handbook-for-devs.html",
  "logo": "https://cdn.freecodecamp.org/universal/favicons/favicon.ico",
