import type { SidebarInfoTemplate, SidebarInfoSubgroupTemplate, SidebarYeargroupTemplate } from ".";

const BUILDING_STYLING_TABLES_REACT_TABLE_V7: SidebarInfoSubgroupTemplate = { // 2020-04-02
  text: 'What’s new in React Table v7?',
  collapsible: true,
  icon: 'fa-brands fa-react',
  subPath: 'building-styling-tables-react-table-v7',
  children: [
    'README',
    'new-features-in-react-table-v7',
    'creating-a-basic-table-with-react-table-v7',
    'adding-a-footer',
    'sorting',
    'filtering',
    'pagination',
    'integrating-react-table-v7-with-material-ui'
  ]
}

const BUILD_STRONGLY_TYPED_POLYMORPHIC_COMPONENTS_REACT_TYPESCRIPT: SidebarInfoSubgroupTemplate = { // 2022-05-25
  text: 'Build strongly typed polymorphic components with React and TypeScript',
  collapsible: true,
  icon: 'fa-brands fa-react',
  subPath: 'build-strongly-typed-polymorphic-components-react-typescript',
  children: [
    'README',
    'real-world-examples-polymorphic-components',
    'building-simple-polymorphic-component',
    'problems-simple-implementation',
    'how-to-use-typescript-build-strongly-typed-polymorphic-components-react',
    'handling-valid-component-attributes-typescript-generics',
    'handling-default-as-attributes',
    'making-component-reusable-props',
    'strictly-omitting-generic-component-props',
    'create-reusable-utility-polymorphic-types',
    'supporting-refs-polymorphic-components',
  ]
}

const HOW_TO_CREATE_CUSTOM_TOAST_COMPONENT_REACT: SidebarInfoSubgroupTemplate = { // 2023-06-21
  text: 'How to create a custom toast component with React',
  collapsible: true,
  icon: 'fa-brands fa-react',
  subPath: 'how-to-create-custom-toast-component-react',
  children: [
    'README',
    'setting-up-our-react-application',
    'selecting-icons-for-our-toast-notifications',
    'creating-a-react-toast-component',
    'structuring-toast-list-components',
    'creating-the-toastlist-component-react',
    'implementing-toast-and-toastlist-components',
    'displaying-toast-notifications-using-the-showtoast-method',
    'deleting-toast-notifications',
    'positioning-toast-notifications',
  ]
}

const MERN_STACK_TUTORIAL: SidebarInfoSubgroupTemplate = { // 2023-11-29
  text: 'The MERN stack: A practical tutorial',
  collapsible: true,
  icon: 'fa-brands fa-react',
  subPath: 'mern-stack-tutorial',
  children: [
    'README',
  ]
}

const HOW_TO_USE_CSS_VARIABLES: SidebarInfoSubgroupTemplate = { // 2025-03-20
  text: 'How to use CSS variables like a pro',
  collapsible: true,
  icon: 'fa-brands fa-css3-alt',
  subPath: 'how-to-use-css-variables',
  children: [
    'README',
    'how-to-declare-and-use-css-variables',
    'project-1-building-button-variations',
    'project-2-setting-css-variables-dynamically-with-javascript',
    'project-3-handling-responsive-design-features',
    'project-4-generating-javascript-free-dynamic-elements',
    'css-variables-vs-sass-variables',
    'advanced-techniques-using-css-variables-in-animations',
    'browser-support-for-the-css-variables-feature',
    'common-mistakes-troubleshooting-css-variables',
  ]
}

const Y2018: SidebarYeargroupTemplate = {
  text: '2018',
  collapsible: true,
  children: [
    // END: 2018
    "using-recompose-to-write-clean-higher-order-components", // 2018-06-25
    "how-css-works-understanding-the-cascade", // 2018-05-29
    "how-css-works-parsing-painting-css-in-the-critical-rendering-path", // 2018-04-10
  ]
}

const Y2019: SidebarYeargroupTemplate = {
  text: '2019',
  collapsible: true,
  children: [
    // END: 2019
    "the-history-and-legacy-of-jquery", // 2019-08-13
    "how-to-manipulate-css-colors-with-javascript", // 2019-03-12
  ]
}

const Y2020: SidebarYeargroupTemplate = {
  text: '2020',
  collapsible: true,
  children: [
    // END: 2020
    "how-the-virtual-dom-works-in-vue-js", // 2020-12-03
    "why-you-should-adopt-react-hooks-instead-of-classes", // 2020-11-12
    "understanding-asynchronous-javascript", // 2020-11-06
    "benchmarking-bundlers-2020-rollup-parcel-webpack", // 2020-10-07
    "methods-for-deep-cloning-objects-in-javascript", // 2020-10-01
    "web-animation-with-html-css-and-javascript", // 2020-09-30
    "react-reference-guide-refs-dom", // 2020-09-04
    "react-reference-guide-context-api", // 2020-09-04
    "understanding-promise-all-in-javascript", // 2020-08-20
    "getting-started-with-vite", // 2020-07-03
    "javascript-concepts-before-learning-react", // 2020-06-23
    "comparing-javascript-animation-libraries", // 2020-04-22
    BUILDING_STYLING_TABLES_REACT_TABLE_V7, // 2020-04-02
    "colors-in-css-present-and-future", // 2020-01-15
  ]
}

const Y2021: SidebarYeargroupTemplate = {
  text: '2021',
  collapsible: true,
  children: [
    // END: 2021
    "whats-new-django-v4", // 2021-12-30
    "making-django-migrations-python", // 2021-12-17
    "why-your-css-fails", // 2021-12-01
    "crud-golang-cockroachdb", // 2021-11-16
    "fundamentals-functional-programming-react", // 2021-11-15
    "building-responsive-components-chakra-ui", // 2021-11-05
    "using-axios-all-make-concurrent-requests", // 2021-10-26
    "animations-react-hooks-greensock", // 2021-10-13
    "controlled-vs-uncontrolled-components-in-react", // 2021-09-29
    "detect-click-outside-react-component-how-to", // 2021-09-24
    "css-margin-vs-padding", // 2021-08-24
    "5-things-not-do-building-react-applications", // 2021-08-13
    "using-setstate-react-components", // 2021-07-16
    "level-up-your-css-selector-skills", // 2021-07-01
    "guide-to-css-animation-for-javascript-developers", // 2021-06-23
    "create-react-app-a-quick-setup-guide", // 2021-06-08
    "a-deep-dive-into-css-modules", // 2021-06-07
    "usestate-vs-useref", // 2021-05-24
    "react-hooks-state-management", // 2021-05-11
    "using-state-machines-with-xstate-and-react", // 2021-05-04
    "understanding-common-frustrations-react-hooks", // 2021-04-26
    "javascript-generators-the-superior-async-await", // 2021-04-14
    "es-modules-in-node-today", // 2021-03-03
    "react-memo-vs-usememo", // 2021-02-18
    "using-storybook-to-develop-react-components-faster", // 2021-02-15
    "managing-react-state-zustand", // 2021-02-14
    "immer-and-immutable-js-how-do-they-compare", // 2021-01-12
    "avoiding-common-mistakes-in-react-hooks", // 2021-01-07
  ]
}

const Y2022: SidebarYeargroupTemplate = {
  text: '2022',
  collapsible: true,
  children: [
    // END: 2022
    "build-inline-editable-ui-react", // 2022-12-15
    "documenting-express-js-api-swagger", // 2022-12-09
    "creating-react-context-menu", // 2022-12-05
    "build-react-dashboard-tremor", // 2022-11-23
    "react-createref-guide", // 2022-11-15
    "immutability-react-should-you-mutate-objects", // 2022-10-14
    "a-guide-to-css-pseudo-elements", // 2022-09-29
    "hsl-hsla-vs-rgb-rgba-css", // 2022-09-16
    "deep-dive-css-individual-transform-properties", // 2022-09-09
    "guide-choosing-right-react-state-management-solution", // 2022-09-09
    "css-style-button-accessibility", // 2022-09-07
    "full-stack-bud-go", // 2022-08-31
    "virtual-dom-react", // 2022-08-16
    "using-react-django-create-app-tutorial", // 2022-07-15
    "using-suspense-react-query-tutorial-examples", // 2022-07-14
    "render-large-lists-react-5-methods-examples", // 2022-07-14
    "build-react-reusable-components-faster-aspect", // 2022-07-13
    "build-task-tracker-solidjs-typescript", // 2022-06-29
    "complete-guide-abortcontroller-node-js", // 2022-06-06
    BUILD_STRONGLY_TYPED_POLYMORPHIC_COMPONENTS_REACT_TYPESCRIPT, // 2022-05-25
    "git-workflow-strategies-multiple-teams-single-repository", // 2022-04-28
    "dockerizing-a-django-app", // 2022-04-08
    "exploring-css-color-module-level-5", // 2022-04-08
    "css-important-declaration-when-why", // 2022-04-06
    "definitive-guide-typing-functions-typescript", // 2022-04-06
    "copy-objects-in-javascript-complete-guide", // 2022-03-09
    "build-react-custom-component-library-theme-ui", // 2022-03-03
    "improve-react-ux-skeleton-ui", // 2022-02-28
    "deep-dive-css-specificity", // 2022-02-24
    "intercepting-javascript-fetch-api-requests-responses", // 2022-02-18
    "control-css-cascade-with-cascade-layers", // 2022-02-16
    "shine-effect-only-css", // 2022-01-25
    "advanced-guide-setting-colors-css", // 2022-01-18
    "solving-prop-drilling-react-apps", // 2022-01-14
    "organizing-express-js-project-structure-better-productivity", // 2022-01-10
  ]
}

const Y2023: SidebarYeargroupTemplate = {
  text: '2023',
  collapsible: true,
  children: [
    // END: 2023
    "advanced-page-transitions-next-js-framer-motion", // 2023-12-28
    "using-strongly-typed-vs-statically-typed-code", // 2023-12-20
    "express-js-adoption-guide", // 2023-12-06
    "vite-adoption-guide", // 2023-11-29
    MERN_STACK_TUTORIAL, // 2023-11-29
    "how-to-use-keyof-operator-typescript", // 2023-11-27
    "best-practices-ci-cd-pipeline-frontend", // 2023-11-08
    "using-css-content-visibility-boost-rendering-performance", // 2023-11-08
    "fetch-api-node-js", // 2023-11-01
    "complete-guide-react-refs", // 2023-10-24
    "useeffect-react-hook-complete-guide", // 2023-10-12
    "top-react-ui-libraries-kits", // 2023-10-05
    "understanding-react-higher-order-components", // 2023-09-19
    "react-toast-libraries-compared", // 2023-09-15
    "comparing-schema-validation-libraries-zod-vs-yup", // 2023-09-05
    "data-fetching-react-suspense", // 2023-08-03
    "modern-guide-react-state-patterns", // 2023-07-31
    "build-react-typescript-app-vite", // 2023-07-21
    "react-useeffect-vs-uselayouteffect-hooks-examples", // 2023-07-12
    "best-headless-cms-platforms", // 2023-07-05
    "declaring-jsx-types-typescript-5-1", // 2023-06-26
    HOW_TO_CREATE_CUSTOM_TOAST_COMPONENT_REACT, // 2023-06-21
    "building-reusable-ui-components-with-react-hooks", // 2023-05-16
    "implement-react-state-management-signia", // 2023-05-10
    "building-react-modal-module-with-react-router", // 2023-05-03
    "understanding-prioritize-react-ui-updates", // 2023-04-17
    "building-adaptive-accessible-ui-library-react-aria", // 2023-04-04
    "getting-started-with-create-react-app", // 2023-03-28
    "when-not-to-use-usememo-react-hook", // 2023-03-10
    "exploring-anime-js-example-site-animation-project", // 2023-03-06
    "rendering-large-lists-react-virtualized", // 2023-03-01
    "react-hooks-cheat-sheet-solutions-common-problems", // 2023-02-20
    "build-powerful-react-forms-with-informed", // 2023-02-09
    "using-react-popper-styled-components", // 2023-02-07
    "complete-guide-threads-node-js", // 2023-01-27
    "creating-typewriter-animation-css", // 2023-01-12
  ]
}

const Y2024: SidebarYeargroupTemplate = {
  text: '2024',
  collapsible: true,
  children: [
    // END: 2024
    "5-ways-rapidly-improve-mysql-database-performance", // 2024-12-29
    "http-requests-axios", // 2024-11-27
    "react-onclick-event-handlers-guide", // 2024-11-26
    "axios-vs-fetch-best-http-requests", // 2024-11-21
    "build-micro-frontend-application-react", // 2024-11-04
    "build-component-library-react-typescript", // 2024-10-23
    "best-react-select-component-libraries", // 2024-10-23
    "building-microservices-node-js", // 2024-10-22
    "build-rest-api-node-express-mysql", // 2024-10-21
    "understanding-redux-tutorial-examples", // 2024-10-17
    "six-things-you-may-not-know-about-javascript", // 2024-10-16
    "react-usereducer-hook-ultimate-guide", // 2024-10-10
    "guide-usestate-react", // 2024-10-08
    "how-when-to-force-react-component-re-render", // 2024-09-25
    "use-forwardref-react", // 2024-09-24
    "commonjs-vs-es-modules-node-js", // 2024-06-06
    "understanding-css-revert-layer", // 2024-04-24
    "practical-guide-typescript-decorators", // 2024-03-21
    "django-rest-framework-create-api", // 2024-03-13
    "modern-api-data-fetching-methods-react", // 2024-03-01
    "building-high-performance-ecommerce-sites-astro", // 2024-02-28
    "understanding-axios-get-requests", // 2024-02-09
    "5-ways-make-http-requests-node-js", // 2024-02-02
    "crud-rest-api-node-js-express-postgresql", // 2024-01-26
    "how-to-animate-svg-css-tutorial-examples", // 2024-01-11
    "react-conditional-rendering-9-methods", // 2024-01-10
  ]
}

const Y2025: SidebarYeargroupTemplate = {
  text: '2025',
  collapsible: true,
  children: [
    // END: 2025
    "how-to-add-custom-fonts-react-native", // 2025-04-07
    "react-useref", // 2025-04-07
    "eliminate-render-blocking-resources-css-javascript", // 2025-04-04
    "mocking-complex-apis-mirage-js", // 2025-04-04
    "top-go-frameworks-2025", // 2025-04-03
    "game-development-frontend-building-excalibur-js", // 2025-04-03

    HOW_TO_USE_CSS_VARIABLES, // 2025-03-20
    "css-overlay", // 2025-03-11
    "pure-component-in-react", // 2025-03-03
    "cursor-css-property", // 2025-03-03
    "react-select-comprehensive-guide", // 2025-03-03
    "build-custom-react-native-turbo-module-android", // 2025-02-27
    "curl-measure-rtt", // 2025-02-26
    "react-memo", // 2025-02-26
    "react-usecallback", // 2025-02-26
    "the-virtual-dom-react", // 2025-02-26
    "switch-statements-javascript", // 2025-02-26
    "building-reusable-multi-step-form-react-hook-form-zod", // 2025-02-26
    "javascript-dictionary", // 2025-02-25
    "css-grid-template-columns", // 2025-02-25
    "guide-node-js-readable-streams", // 2025-02-25
    "advanced-react-state-management-using-url-parameters", // 2025-02-24
    "best-react-native-ui-component-libraries", // 2025-02-21
    "docker-alternatives", // 2025-02-21
    "ternary-operator-javascript", // 2025-02-20
    "tsup", // 2025-02-20
    "react-higher-order-components", // 2025-02-20
    "dependency-inversion-principle", // 2025-02-20
    "building-telegram-bot-grammy", // 2025-02-20
    "box-shadow-css", // 2025-02-19
    "complete-guide-typescript-enums-react-native", // 2025-02-19
    "creating-react-animations-with-motion", // 2025-02-18
    "event-loop-and-call-stack-js", // 2025-02-18
    "react-context-tutorial", // 2025-02-17
    "javascript-arrow-functions", // 2025-02-17
    "javascript-null-empty-function", // 2025-02-14
    "mern-stack", // 2025-02-13
    "leveraging-parallel-computing-node-js", // 2025-02-13
    "guide-modern-frontend-architecture-patterns", // 2025-02-12
    "guide-graceful-degradation-web-development", // 2025-02-12
    "building-high-performance-websites-using-htmx-go", // 2025-02-10
    "improve-ux-with-scroll-select-box", // 2025-02-07
    "best-ci-cd-tools-react-native", // 2025-02-05
    "using-bootstrap-react-tutorial-examples", // 2025-01-21
  ]
}

export const template: SidebarInfoTemplate = {
  name: "blog.logrocket.com",
  faviconPath: "/assets/image/blog.logrocket.com/favicon.png",
  linksMap: new Map([
    [
    "java-android", [
      "build-custom-react-native-turbo-module-android", // 2025-02-27
      // END: 2025java-android
      // END: java-android
    ]],[
    "java-spring", [
      "dependency-inversion-principle", // 2025-02-20
      // END: 2025java-spring
      // END: java-spring
    ]],[
    "js", [
      "how-to-manipulate-css-colors-with-javascript", // 2019-03-12
      "the-history-and-legacy-of-jquery", // 2019-08-13
      // END: 2019js
      "understanding-promise-all-in-javascript", // 2020-08-20
      "methods-for-deep-cloning-objects-in-javascript", // 2020-10-01
      "understanding-asynchronous-javascript", // 2020-11-06
      // END: 2020js
      "crud-golang-cockroachdb", // 2021-11-16
      // END: 2021js
      "copy-objects-in-javascript-complete-guide", // 2022-03-09
      // END: 2022js
      // END: 2023js
      "six-things-you-may-not-know-about-javascript", // 2024-10-16
      // END: 2024js
      "improve-ux-with-scroll-select-box", // 2025-02-07
      "javascript-null-empty-function", // 2025-02-14
      "javascript-arrow-functions", // 2025-02-17
      "event-loop-and-call-stack-js", // 2025-02-18
      "ternary-operator-javascript", // 2025-02-20
      "javascript-dictionary", // 2025-02-25
      "switch-statements-javascript", // 2025-02-26
      
      // END: 2025js
      // END: js
    ]],[
    "ts", [
      "javascript-generators-the-superior-async-await", // 2021-04-14
      // END: 2021ts
      "copy-objects-in-javascript-complete-guide", // 2022-03-09
      "definitive-guide-typing-functions-typescript", // 2022-04-06
      // END: 2022ts
      "declaring-jsx-types-typescript-5-1", // 2023-06-26
      "babel-vs-typescript-choosing-right-compiler-project", // 2023-07-10
      "how-to-use-keyof-operator-typescript", // 2023-11-27
      "using-strongly-typed-vs-statically-typed-code", // 2023-12-20
      // END: 2023ts
      "practical-guide-typescript-decorators", // 2024-03-21
      // END: 2024ts
      "guide-graceful-degradation-web-development", // 2025-02-12
      "complete-guide-typescript-enums-react-native", // 2025-02-19
      "building-telegram-bot-grammy", // 2025-02-20
      "dependency-inversion-principle", // 2025-02-20
      "tsup", // 2025-02-20
      // END: 2025ts
      // END: ts
    ]],[
    "js-node", [
      "comparing-javascript-animation-libraries", // 2020-04-22
      "getting-started-with-vite", // 2020-07-03
      // END: 2020js-node
      "immer-and-immutable-js-how-do-they-compare", // 2021-01-12
      "es-modules-in-node-today", // 2021-03-03
      // END: 2021js-node
      "intercepting-javascript-fetch-api-requests-responses", // 2022-02-18
      "complete-guide-abortcontroller-node-js", // 2022-06-06
      // END: 2022js-node
      "complete-guide-threads-node-js", // 2023-01-27
      "exploring-anime-js-example-site-animation-project", // 2023-03-06
      "comparing-schema-validation-libraries-zod-vs-yup", // 2023-09-05
      "fetch-api-node-js", // 2023-11-01
      "best-practices-ci-cd-pipeline-frontend", // 2023-11-08
      "vite-adoption-guide", // 2023-11-29
      // END: 2023js-node
      "commonjs-vs-es-modules-node-js", // 2024-06-06
      "building-microservices-node-js", // 2024-10-22
      // END: 2024js-node
      "5-ways-make-http-requests-node-js", // 2024-02-02
      "leveraging-parallel-computing-node-js", // 2025-02-13
      "guide-node-js-readable-streams", // 2025-02-25
    
      "game-development-frontend-building-excalibur-js", // 2025-04-03
      "mocking-complex-apis-mirage-js", // 2025-04-04
      // END: 2025js-node
      // END: js-node
    ]],[
    "js-react", [
      "using-recompose-to-write-clean-higher-order-components", // 2018-06-25
      // END: 2018js-react
      // END: 2019js-react
      BUILDING_STYLING_TABLES_REACT_TABLE_V7, // 2020-04-02
      "javascript-concepts-before-learning-react", // 2020-06-23
      "react-reference-guide-context-api", // 2020-09-04
      "react-reference-guide-refs-dom", // 2020-09-04
      "benchmarking-bundlers-2020-rollup-parcel-webpack", // 2020-10-07
      "why-you-should-adopt-react-hooks-instead-of-classes", // 2020-11-12
      // END: 2020js-react
      "avoiding-common-mistakes-in-react-hooks", // 2021-01-07
      "managing-react-state-zustand", // 2021-02-14
      "using-storybook-to-develop-react-components-faster", // 2021-02-15
      "react-memo-vs-usememo", // 2021-02-18
      "using-state-machines-with-xstate-and-react", // 2021-05-04
      "understanding-common-frustrations-react-hooks", // 2021-04-26
      "usestate-vs-useref", // 2021-05-24
      "react-hooks-state-management", // 2021-05-11
      "create-react-app-a-quick-setup-guide", // 2021-06-08
      "using-setstate-react-components", // 2021-07-16
      "5-things-not-do-building-react-applications", // 2021-08-13
      "detect-click-outside-react-component-how-to", // 2021-09-24
      "controlled-vs-uncontrolled-components-in-react", // 2021-09-29
      "animations-react-hooks-greensock", // 2021-10-13
      "using-axios-all-make-concurrent-requests", // 2021-10-26
      "building-responsive-components-chakra-ui", // 2021-11-05
      "fundamentals-functional-programming-react", // 2021-11-15
      // END: 2021js-react
      "solving-prop-drilling-react-apps", // 2022-01-14
      "build-react-custom-component-library-theme-ui", // 2022-03-03
      BUILD_STRONGLY_TYPED_POLYMORPHIC_COMPONENTS_REACT_TYPESCRIPT, // 2022-05-25
      "render-large-lists-react-5-methods-examples", // 2022-07-14
      "using-suspense-react-query-tutorial-examples", // 2022-07-14
      "using-react-django-create-app-tutorial", // 2022-07-15
      "virtual-dom-react", // 2022-08-16
      "guide-choosing-right-react-state-management-solution", // 2022-09-09
      "immutability-react-should-you-mutate-objects", // 2022-10-14
      "react-createref-guide", // 2022-11-15
      "build-react-dashboard-tremor", // 2022-11-23
      "creating-react-context-menu", // 2022-12-05
      "build-inline-editable-ui-react", // 2022-12-15
      // END: 2022js-react
      "using-react-popper-styled-components", // 2023-02-07
      "build-powerful-react-forms-with-informed", // 2023-02-09
      "react-hooks-cheat-sheet-solutions-common-problems", // 2023-02-20
      "improve-react-ux-skeleton-ui", // 2022-02-28
      "rendering-large-lists-react-virtualized", // 2023-03-01
      "when-not-to-use-usememo-react-hook", // 2023-03-10
      "getting-started-with-create-react-app", // 2023-03-28
      "building-adaptive-accessible-ui-library-react-aria", // 2023-04-04
      "understanding-prioritize-react-ui-updates", // 2023-04-17
      "building-react-modal-module-with-react-router", // 2023-05-03
      "implement-react-state-management-signia", // 2023-05-10
      "building-reusable-ui-components-with-react-hooks", // 2023-05-16
      HOW_TO_CREATE_CUSTOM_TOAST_COMPONENT_REACT, // 2023-06-21
      "declaring-jsx-types-typescript-5-1", // 2023-06-26
      "react-useeffect-vs-uselayouteffect-hooks-examples", // 2023-07-12
      "build-react-reusable-components-faster-aspect", // 2022-07-13
      "build-react-typescript-app-vite", // 2023-07-21
      "modern-guide-react-state-patterns", // 2023-07-31
      "data-fetching-react-suspense", // 2023-08-03
      "react-toast-libraries-compared", // 2023-09-15
      "understanding-react-higher-order-components", // 2023-09-19
      "top-react-ui-libraries-kits", // 2023-10-05
      "useeffect-react-hook-complete-guide", // 2023-10-12
      // END: 2023js-react
      "react-conditional-rendering-9-methods", // 2024-01-10
      "modern-api-data-fetching-methods-react", // 2024-03-01
      "how-to-use-axios-post-requests", // 2024-07-01
      "use-forwardref-react", // 2024-09-24
      "how-when-to-force-react-component-re-render", // 2024-09-25
      "guide-usestate-react", // 2024-10-08
      "react-usereducer-hook-ultimate-guide", // 2024-10-10
      "understanding-redux-tutorial-examples", // 2024-10-17
      "best-react-select-component-libraries", // 2024-10-23
      "build-component-library-react-typescript", // 2024-10-23
      "complete-guide-react-refs", // 2023-10-24
      "build-micro-frontend-application-react", // 2024-11-04
      "react-onclick-event-handlers-guide", // 2024-11-26
      MERN_STACK_TUTORIAL, // 2023-11-29
      // END: 2024js-react
      "using-bootstrap-react-tutorial-examples", // 2025-01-21
      "best-ci-cd-tools-react-native", // 2025-02-05
      "mern-stack", // 2025-02-13
      "react-context-tutorial", // 2025-02-17
      "creating-react-animations-with-motion", // 2025-02-18
      "complete-guide-typescript-enums-react-native", // 2025-02-19
      "react-higher-order-components", // 2025-02-20
      "best-react-native-ui-component-libraries", // 2025-02-21
      "advanced-react-state-management-using-url-parameters", // 2025-02-24
      "building-reusable-multi-step-form-react-hook-form-zod", // 2025-02-26
      "the-virtual-dom-react", // 2025-02-26
      "react-usecallback", // 2025-02-26
      "react-memo", // 2025-02-26
      "build-custom-react-native-turbo-module-android", // 2025-02-27
      "react-select-comprehensive-guide", // 2025-03-03
      "pure-component-in-react", // 2025-03-03
      "react-useref", // 2025-04-07
      "how-to-add-custom-fonts-react-native", // 2025-04-07
      // END: 2025js-react
      // END: js-react
    ]],[
    "js-next", [
      "advanced-page-transitions-next-js-framer-motion", // 2023-12-28
      // END: 2023js-next
      // END: 2024js-next
      // END: 2025js-next
      // END: js-next
    ]],[
    "js-vue", [
      "how-the-virtual-dom-works-in-vue-js", // 2020-12-03
      // END: 2020js-vue
      // END: 2021js-vue
      // END: 2022js-vue
      // END: 2023js-vue
      // END: 2024js-vue
      // END: 2025js-vue
      // END: js-vue
    ]],[
    "js-svelte", [
      "full-stack-bud-go", // 2022-08-31
      // END: 2022js-svelte
      // END: 2023js-svelte
      // END: 2024js-svelte
      // END: js-svelte
    ]],[
    "js-storybook", [
      // END: 2019js-storybook
      // END: 2020js-storybook
      "using-storybook-to-develop-react-components-faster", // 2021-02-15
      // END: 2021js-storybook
      // END: 2022js-storybook
      // END: 2023js-storybook
      // END: 2024js-storybook
      // END: js-storybook
    ]],[
    "js-solid", [
      "build-task-tracker-solidjs-typescript", // 2022-06-29
      // END: 2022js-solid
      // END: 2023js-solid
      // END: 2024js-solid
      // END: 2025js-solid
      // END: js-solid
    ]],[
    "js-axios", [
      "using-axios-all-make-concurrent-requests", // 2021-10-26
      // END: 2021js-axios
      // END: 2022js-axios
      // END: 2023js-axios
      "understanding-axios-get-requests", // 2024-02-09
      "how-to-use-axios-post-requests", // 2024-07-01
      "axios-vs-fetch-best-http-requests", // 2024-11-21
      "http-requests-axios", // 2024-11-27
      // END: 2024js-axios
      // END: js-axios
    ]],[
    "js-express", [
      // END: 2020js-express
      // END: 2021js-express
      "organizing-express-js-project-structure-better-productivity", // 2022-01-10
      "documenting-express-js-api-swagger", // 2022-12-09
      // END: 2022js-express
      MERN_STACK_TUTORIAL, // 2023-11-29
      "express-js-adoption-guide", // 2023-12-06
      // END: 2023js-express
      "crud-rest-api-node-js-express-postgresql", // 2024-01-26
      "build-rest-api-node-express-mysql", // 2024-10-21
      // END: 2024js-express
      // END: 2025js-express
      // END: js-express
    ]],[
    "js-astro", [
      "best-headless-cms-platforms", // 2023-07-05
      // END: 2023js-astro
      "building-high-performance-ecommerce-sites-astro", // 2024-02-28
      // END: 2024js-astro
      // END: 2025js-astro
      // END: js-astro
    ]],[
    "py", [
      "dependency-inversion-principle", // 2025-02-20
      // END: 2025py
      // END: py
    ]],[
    "py-django", [
      "making-django-migrations-python", // 2021-12-17
      "whats-new-django-v4", // 2021-12-30
      // END: 2021py-django
      "dockerizing-a-django-app", // 2022-04-08
      "using-react-django-create-app-tutorial", // 2022-07-15
      // END: 2022py-django
      "django-rest-framework-create-api", // 2024-03-13
      // END: 2024py-django
      // END: py-django
    ]],[
    "cs", [
      // END: 2024cs
      "dependency-inversion-principle", // 2025-02-20
      // END: 2025cs
      // END: cs
    ]],[
    "css", [
      "how-css-works-parsing-painting-css-in-the-critical-rendering-path", // 2018-04-10
      "how-css-works-understanding-the-cascade", // 2018-05-29
      // END: 2018css
      "how-to-manipulate-css-colors-with-javascript", // 2019-03-12
      // END: 2019css
      "colors-in-css-present-and-future", // 2020-01-15
      "web-animation-with-html-css-and-javascript", // 2020-09-30
      // END: 2020css
      "a-deep-dive-into-css-modules", // 2021-06-07
      "guide-to-css-animation-for-javascript-developers", // 2021-06-23
      "level-up-your-css-selector-skills", // 2021-07-01
      "css-margin-vs-padding", // 2021-08-24
      "why-your-css-fails", // 2021-12-01
      // END: 2021css
      "advanced-guide-setting-colors-css", // 2022-01-18
      "shine-effect-only-css", // 2022-01-25
      "control-css-cascade-with-cascade-layers", // 2022-02-16
      "deep-dive-css-specificity", // 2022-02-24
      "css-important-declaration-when-why", // 2022-04-06
      "exploring-css-color-module-level-5", // 2022-04-08
      "css-style-button-accessibility", // 2022-09-07
      "deep-dive-css-individual-transform-properties", // 2022-09-09
      "hsl-hsla-vs-rgb-rgba-css", // 2022-09-16
      "a-guide-to-css-pseudo-elements", // 2022-09-29
      // END: 2022css
      "creating-typewriter-animation-css", // 2023-01-12
      "using-css-content-visibility-boost-rendering-performance", // 2023-11-08
      // END: 2023css
      "how-to-animate-svg-css-tutorial-examples", // 2024-01-11
      "understanding-css-revert-layer", // 2024-04-24
      // END: 2024css
      "improve-ux-with-scroll-select-box", // 2025-02-07
      "box-shadow-css", // 2025-02-19
      "css-grid-template-columns", // 2025-02-25
      "cursor-css-property", // 2025-03-03
      "css-overlay", // 2025-03-11
      HOW_TO_USE_CSS_VARIABLES, // 2025-03-20

      "eliminate-render-blocking-resources-css-javascript", // 2025-04-04
      // END: 2025css
      // END: css
    ]],[
    "go", [
      // END: 2020go
      "crud-golang-cockroachdb", // 2021-11-16
      // END: 2021go
      "full-stack-bud-go", // 2022-08-31
      // END: 2022go
      // END: 2023go
      // END: 2024go
      "building-high-performance-websites-using-htmx-go", // 2025-02-10
      "docker-alternatives", // 2025-02-21
      // END: 2025go
      // END: go
    ]],[
    "c", [
      "using-strongly-typed-vs-statically-typed-code", // 2023-12-20
      // END: 2023c
      // END: 2024c
      // END: 2025c
      // END: c
    ]],[
    "git", [
      "git-workflow-strategies-multiple-teams-single-repository", // 2022-04-28
      // END: 2022git
      // END: 2023git
      // END: 2024git
      // END: 2025git
      // END: git
    ]],[
    "github", [
      "best-practices-ci-cd-pipeline-frontend", // 2023-11-08
      // END: 2023github
      // END: 2024github
      // END: 2025github
      // END: github
    ]],[
    "docker", [
      "dockerizing-a-django-app", // 2022-04-08
      // END: 2022docker
      // END: 2023docker
      // END: 2024docker
      "docker-alternatives", // 2025-02-21
      // END: 2025docker
      // END: docker
    ]],[
    "podman", [
      // END: 2022podman
      // END: 2023podman
      // END: 2024podman
      "docker-alternatives", // 2025-02-21
      // END: 2025podman
      // END: podman
    ]],[
    "jenkins", [
      "best-ci-cd-tools-react-native", // 2025-02-05
      // END: 2025jenkins
      // END: jenkins
    ]],[
    "mysql", [
      "build-rest-api-node-express-mysql", // 2024-10-21
      "5-ways-rapidly-improve-mysql-database-performance", // 2024-12-29
      // END: 2024mysql
      // END: 2025mysql
      // END: mysql
    ]],[
    "postgresql", [
      // END: 2023postgresql
      "crud-rest-api-node-js-express-postgresql", // 2024-01-26
      // END: 2024postgresql
      // END: 2025postgresql
      // END: 2026postgresql
      // END: postgresql
    ]],[
    "mongodb", [
      // END: 2022mongodb
      MERN_STACK_TUTORIAL, // 2023-11-29
      // END: 2023mongodb
      "building-microservices-node-js", // 2024-10-22
      // END: 2024mongodb
      // END: 2025mongodb
      // END: mongodb
    ]],[
    "cockroach", [
      // END: 2020cockroach
      "crud-golang-cockroachdb", // 2021-11-16
      // END: 2021cockroach
      // END: 2022cockroach
      // END: 2023cockroach
      // END: 2024cockroach
      // END: 2025cockroach
      // END: 2025cockroach
      // END: cockroach
    ]],[
    "curl", [
      "curl-measure-rtt", // 2025-02-26
      // END: 2025curl
      // END: curl
    ]],[
    "system-design", [
      // END: 2024system-design
      "dependency-inversion-principle", // 2025-02-20
      // END: 2025system-design
      // END: system-design
    ]],[
    "all", [
      Y2025,
      Y2024,
      Y2023,
      Y2022,
      Y2021,
      Y2020,
      Y2019,
      Y2018,
    ]]
  ])
}