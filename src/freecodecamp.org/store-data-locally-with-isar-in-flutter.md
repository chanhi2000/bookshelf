...
Open your <FontIcon icon="iconfont icon-yaml"/>`pubspec.yaml` file and add the following:
...
Create a file named <FontIcon icon="fa-brands fa-dart-lang"/>`isar_setup.dart`. This will handle the opening of the Isar database.
...
Now let’s define our data model for tasks. Create a file named <FontIcon icon="fa-brands fa-dart-lang"/>`task.dart`.
...
This generates <FontIcon icon="fa-brands fa-dart-lang"/>`task.g.dart`, which contains the necessary schema code.

---

## How to Build the Repository for CRUD Operations

Create a new file called <FontIcon icon="fa-brands fa-dart-lang"/>`task_repository.dart`. This will house the methods for interacting with the database.
...
Now, let’s connect everything inside <FontIcon icon="fa-brands fa-dart-lang"/>`main.dart`.
