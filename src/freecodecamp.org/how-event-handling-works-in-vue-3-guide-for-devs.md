...
- `defineEmits()`: This is used in the child component to define custom events. Here, the child emits a `custom-event` with a payload of `'Hello from ChildComponent'`. ([<FontIcon icon="fas fa-globe"/>you can learn more details of emit from here](https://asfakahmedsblog.hashnode.dev/understanding-vuejs-emit-a-complete-guide))
...
- `v-model="formData.name"`: This binds the input field directly to the `formData.name` variable, allowing automatic updates as the user types. ([<FontIcon icon="fas fa-globe"/>you can learn more details of `v-model` from here](https://asfakahmedsblog.hashnode.dev/understanding-vuejs-v-model-a-complete-guide))
