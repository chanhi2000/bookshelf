...
- Flutter SDK installed and configured ([<FontIcon icon="iconfont icon-flutter"/>Flutter installation guide](https://docs.flutter.dev/get-started/install)).
...
Navigate into the project folder and open the <FontIcon icon="iconfont icon-yaml"/>`pubspec.yaml` file. Add the following dependencies:
...
For scalability, keep your project organized. Create the following folder structure inside <FontIcon icon="fas fa-folder-open"/>`lib`:
...
This creates <FontIcon icon="fa-brands fa-dart-lang"/>`app_route.gr.dart` in the `route_config` folder. The file includes strongly typed classes for each screen, such as `Controlscreen`, `Screen1`, `Screen2`, and `Screen3`.
...
## Step 5: Setting Up AutoRoute in <FontIcon icon="fa-brands fa-dart-lang"/>`main.dart`

In <FontIcon icon="fa-brands fa-dart-lang"/>`main.dart`, configure the app to use AutoRoute:
...
### <FontIcon icon="fa-brands fa-dart-lang"/>`screen1.dart`
...
1. `@RoutePage(name: 'Screen1')`
    ...
    - The `name` parameter gives AutoRoute a clear identifier for the screen, which will also be reflected in the generated <FontIcon icon="fa-brands fa-dart-lang"/>`app_route.gr.dart` file.
...
- Create <FontIcon icon="fa-brands fa-dart-lang"/>`screen2.dart` and <FontIcon icon="fa-brands fa-dart-lnag"/>`screen3.dart` inside `sub_screens`.
...
The **ControlScreen** acts as the entry point of our app. It’s the first screen that loads when the app starts (because in <FontIcon icon="fa-brands fa-dart-lang"/>`app_route.dart`, we set `/` > `Controlscreen`).
...
- AutoRoute will generate a `Controlscreen.page` entry for use in <FontIcon icon="fa-brands fa-dart-lang"/>`app_route.dart`.
...
- `pushNamed('/screen1')` matches the path we defined earlier in <FontIcon icon="fa-brands fa-dart-lang"/>`app_route.dart`:
...
3. **AutoRoute matches path**: It looks up `/screenX` in the route list we defined in <FontIcon icon="fa-brands fa-dart-lang"/>`app_route.dart`.
4. **Generated code takes over**: <FontIcon icon="fa-brands fa-dart-lang"/>`app_route.gr.dart` (generated by AutoRoute) creates and pushes the correct screen widget onto the stack.
