...
Before you can build an expense tracker, you need real transaction data. Instead of creating a CSV from scratch, let’s use a dataset from Kaggle: [<FontIcon icon="iconfont icon-kaggle"/>My Expenses Data by Tharun Prabu](https://kaggle.com/datasets/tharunprabu/my-expenses-data).
...
2. Storing Expenses in a CSV File
    - Each new entry is saved back into <FontIcon icon="fas fa-file-csv"/>`expense_data_1.csv`, so your history doesn’t disappear when you restart the app.
