...
If we are determined that targeted elements of this new selector won’t pollute the [**light DOM**](/blog.master.dev/light-dom-only.md) with extra markup, then we have to hide that markup in the shadow DOM. If we do that, then the closest I know of to a custom pseudo-element is the [**`::part`**](/blog.master.dev/light-dom-only.md) pseudo-element. If we use that, then by design, we can’t use:
