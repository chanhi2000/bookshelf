...
If, however, the hue interpolation method is set to `decreasing`, the minutes hand will always go in a counterclockwise direction. As the [<FontIcon icon="fas fa-globe"/>specification says](https://drafts.csswg.org/css-color/#hue-increasing), *“depending on the difference between the two angles, this will either look the same as shorter or as longer.”*
...
Reading about [<FontIcon icon="iconfont icon-css-tricks"/>`color-mix()`](https://css-tricks.com/almanac/functions/c/color-mix/) in the CSS-Tricks Almanac will give you a basic idea of how this is done, but if you’re like me and want the raw code, here it is:
