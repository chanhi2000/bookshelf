...
With CSS Anchor Positioning, we can attach an absolutely-positioned element (that we may call a “target”) to another element (that we may call an “anchor”). This makes creating things like tooltips, modals, and pop-ups trivial in CSS, although it can be used for a [**variety of layouts**](/blog.master.dev/drawing-a-line-to-connect-elements-with-css-anchor-positioning.md).
