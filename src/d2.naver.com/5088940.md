...
오픈소스 라이브러리를 개발하는 개발자로서 타입에 대한 전문성은 필수불가결합니다. 실제로 저는 라이브러리 개발 도중, 위에서 풀어본 문제를 능가하는 난이도의 타입을 다룬 적도 있습니다. 이 모든 것은 지적인 현학성을 추구하기 위함도, [<FontIcon icon="fas fa-gobe"/>아무도 읽지 못하는 무기를 개발하기 위함](https://m.hanbit.co.kr/store/books/book_view.html?p_code=E2375873090)도 아닙니다. 그저 라이브러리의 사용성을 증대시키고 더 나은 도구를 만들기 위함입니다.
