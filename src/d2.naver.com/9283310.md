...
'infer, never만 보면 두려워지는 당신을 위한 타입 추론' 관련 영상은 [<VPIcon icon="fas fa-globe"/>링크](https://naver.me/Gq8Plxdp)에서 보실 수 있습니다.
...
타입 이론의 근간에는 [<VPIcon icon="fa-brands fa-wikipedia-w"/>수학기초론(foundation of mathematics)](https://en.wikipedia.org/wiki/Foundations_of_mathematics)이 있습니다. 20세기 이전까지만 하더라도 수학은 생각보다 엄밀하지 않았는데요, 1908년 [<VPIcon icon="fa-brands fa-wikipedia-w"/>러셀의 역설](https://en.wikipedia.org/wiki/Russell%27s_paradox)이 발견되며 모든 수학의 기초인 집합이 흔들립니다. 이 문제를 해결하고자 ZFC 공리계, 괴델의 불완전성 정리 등의 엄밀한 고찰이 계속되었습니다.

![러셀의 역설의 발견](/assets/image/d2.naver.com/9283310/russell.png)

타입 이론은 위대한 과학자 중 하나인 [<VPIcon icon="fa-brands fa-wikipedia-w"/>앨런조 처치(Alanzo Church)](https://en.wikipedia.org/wiki/Alonzo_Church)가 [<VPIcon icon="fa-brands fa-wikipedia-w"/>람다 대수(lambda calculus)](https://en.wikipedia.org/wiki/Lambda_calculus)를 고찰하면서 탄생했습니다. 처치 외에도 많은 수학자가 유사한 시도를 했고, 비교적 최근까지도 학계에서 다뤄지는 주제라고 합니다. 수리철학적으로 더 자세한 내용을 알고 싶다면 [<VPIcon icon="fas fa-globe"/>스탠포드 대학의 자료](https://plato.stanford.edu/entries/type-theory/)를 참고해 주세요.

---

## TypeScript와 타입 이론

TS 1.8까지는 [<VPIcon icon="fas fa-globe"/>공식 문서](https://javascript.xgqfrms.xyz/pdfs/TypeScript%20Language%20Specification.pdf)가 존재했으나, 이후 Microsoft가 문서 유지보수를 포기했습니다. 때문에 현재 TS의 사양은 TypeScript 컴파일러(이하 tsc)의 구현 그 자체가 된 상황입니다. 게다가 구버전 문서에도 일관성 있는 타입 추론 알고리즘에 대한 정보는 찾아보기가 어렵습니다.
...
특별히 a ≥ b이면서 a ≤ b인 관계를 a = b라고 합니다. 이런 집합을 [<VPIcon icon="fa-brands fa-wikipedia-w"/>전순서집합(totally ordered set)](https://en.wikipedia.org/wiki/Total_order)이라고 합니다. 엄밀하게는 몇 가지 조건이 붙지만, 지금은 중요하지 않으니 참고만 해주세요.
...
특별히 a ≳ b 이면서 a ≲ b 인 경우를 a ≃ b 라고 표현하겠습니다. 여기서 중요한 점은 a ≳ b가 아니라고 해서 a ≲ b라고 할 수 없다는 점입니다. 이런 집합을 [<VPIcon icon="fa-brands fa-wikipedia-w"/>부분순서집합(partially ordered set)](https://en.wikipedia.org/wiki/Partially_ordered_set)이라고 합니다.
