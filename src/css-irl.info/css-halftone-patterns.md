...
description: "Article(s) > CSS Halftone Patterns"
...
head:
  - - meta:
    - property: og:title
      content: "Article(s) > CSS Halftone Patterns"
    - property: og:description
      content: "CSS Halftone Patterns"
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/css-irl.info/css-halftone-patterns.html
...
A little while ago, [Ana Tudor (<VPIcon icon="fa-brands fa-codepen"/>`thebabydino`)](https://codepen.io/thebabydino) created an impressive collection of [halftone patterns using only CSS (<VPIcon icon="fa-brands fa-codepen" />`thebabydino`)](https://codepen.io/thebabydino/pen/gOpMeWv). As I had a little time to spare, I thought I’d dig into the code and see how it was done! Ana’s demos are made using Sass — what better way to learn than to try to produce similar effects using vanilla CSS?

![One of Ana Tudor’s beautiful halftone patterns created with CSS](https://css-irl.info/css-halftone-patterns-01a.jpg)

Ana’s demo uses [<VPIcon icon="fa-brands fa-firefox"/>Houdini](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Houdini) to animate the halftone background (only supported in Chromium browsers). For the purpose of this article we’re just going to be concentrating on the halftone pattern itself, rather than the animation.
...
![Photograph from Unsplash by Regine Tholen, converted to halftone](https://css-irl.info/css-halftone-patterns-01b.jpg)
...
We’ll have just a single circle at this point. If we change the background size to something smaller, we’ll instead get a repeating dotted pattern. We can use `background-repeat: round` to ensure the dots won’t be clipped, and we’ll position our background in the centre. I prefer to use separate properties as it’s easier to read, but you can use the `background` [<VPIcon icon="fa-brands fa-firefox"/>shorthand](https://developer.mozilla.org/en-US/docs/Web/CSS/background).
...
<CodePen
  user="michellebarker"
  slug-hash="eYrKrKJ"
  title="Simple halftone backgrounds"
  :default-tab="['css','result']"
  :theme="dark"/>
...
![With the `multiply` blend mode](https://css-irl.info/css-halftone-patterns-05.jpg)

![With the `screen` blend mode](https://css-irl.info/css-halftone-patterns-06.jpg)
...
<CodePen
  user="michellebarker"
  slug-hash="YzLvvbm"
  title="Halftone background with blend modes"
  :default-tab="['css','result']"
  :theme="dark"/>
...
Ana has [another demo (<VPIcon icon="fa-brands fa-codepen" />`thebabydino`)](https://codepen.io/thebabydino/pen/LYGGwrm) that applies a halftone treatment to non-transparent images. We’re not going to walk through it here, but it’s another intriguing one to study!
...
```component VPCard
{
  "title": "CSS Halftone Patterns",
  "desc": "undefined",
  "link": "https://chanhi2000.github.io/bookshelf/css-irl.info/css-halftone-patterns.html",
  "logo": "https://css-irl.info/favicon/apple-touch-icon.png",
