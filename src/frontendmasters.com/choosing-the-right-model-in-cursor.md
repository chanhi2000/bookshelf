...
A number of the big players are coming out with their own AI coding assistants (e.g., [<FontIcon icon="iconfont icon-openai"/>OpenAI’s Codex](https://openai.com/codex/), [<FontIcon icon="iconfont icon-claude"/>Anthropic’s Claude Code](https://anthropic.com/claude-code), and [<FontIcon icon="iconfont icon-gcp"/>Google Gemini CLI](https://cloud.google.com/gemini/docs/codeassist/gemini-cli)). However, one of the advantages of using a third-party tool like Cursor is that you have the option to choose from a wide selection of models. The downside—of course—is that, like Uncle Ben would always say, “With great power comes great responsibility.”
...
If you *really* want to level up with your AI coding skills, you should go from here right to Steve’s course: [<FontIcon icon="fas fa-globe"/>Cursor & Claude Code: Professional AI Setup](https://frontendmasters.com/courses/pro-ai/).
