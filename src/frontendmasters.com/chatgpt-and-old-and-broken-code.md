...
Then I’m not a fan of those `body` styles. Firstly, without zeroing its [def<FontIcon icon="fas fa-globe"/>ault](https://miriamsuzanne.com/2022/07/04/body-margin-8px/) `margin`, setting its height to `100vh` creates a scrollbar. Secondly, it [<FontIcon icon="fa-brands fa-chrome"/>can also be problematic](https://issues.chromium.org/issues/41390045#comment5) even when setting `margin: 0`. Just don’t do it and do this instead, it has better support than `dvh`:
