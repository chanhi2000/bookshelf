---
lang: en-US
title: "Form-Associated Custom Elements in Practice"
description: "Article(s) > Form-Associated Custom Elements in Practice"
...
head:
  - - meta:
    - property: og:title
      content: "Article(s) > Form-Associated Custom Elements in Practice"
    - property: og:description
      content: "Form-Associated Custom Elements in Practice"
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/frontendmasters.com/form-associated-custom-elements-in-practice.html
...
cover: https://frontendmasters.com/blog/wp-json/social-image-generator/v1/image/8896
---
...
<SiteInfo
  name="Form-Associated Custom Elements in Practice"
  desc="When you make a Web Component for a form element, you've got a bit of extra work to do to make sure they participate on the form in expected ways."
  url="https://frontendmasters.com/blog/form-associated-custom-elements-in-practice/"
  logo="https://frontendmasters.com/favicon.ico"
  preview="https://frontendmasters.com/blog/wp-json/social-image-generator/v1/image/8896"/>

I was well over three-quarters of the way through [**rewriting the AgnosticUI components in Lit**](/frontendmasters.com/post-mortem-rewriting-agnosticui-with-lit-web-components.md) when I realized I had a massive blind spot. My `<ag-input>` looked solid and fired events correctly, but it lacked [<VPIcon icon="fa-brands fa-firefox"/>Form-Associated Custom Element](https://developer.mozilla.org/en-US/docs/Web/API/ElementInternals) (FACE) support. This meant it was essentially invisible to native `<form>` submissions.

I was completely unaware of this until a conversation with my friend [Marc van Neerven (<VPIcon icon="fa-brands fa-linkedin"/>`mvneerven`)](https://linkedin.com/in/mvneerven/). We were discussing the nuances of Web Components and [<VPIcon icon="fa-brands fa-firefox"/>Shadow DOM](https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_shadow_DOM) when Marc pointed out the importance of **form association**.

Fueled by the mild embarrassment of having missed something so fundamental, I immediately started digging through articles like [<VPIcon icon="fa-brands fa-safari"/>`ElementInternals` and Form-Associated Custom Elements](https://webkit.org/blog/13711/elementinternals-and-form-associated-custom-elements/) to understand how this *Form-Associated Custom Element* stuff worked. Of course, I understood that native HTML form controls have built-in submission logic and `FormData` support, but I had absolutely no idea a FACE API even existed.
...
![Comparison of native input and custom element in a form, showing how the native input correctly captures the value 'hello' in FormData, while the custom element results in empty FormData.](https://i0.wp.com/frontendmasters.com/blog/wp-content/uploads/2026/03/558955386-ada204cb-7ec0-43b2-824a-755a29a558cc.png?resize=1024%2C626&ssl=1)
...
Enabling FACE starts with a deceptive bit of boilerplate. You tell the browser your element wants to participate in forms, and then you grab a handle to the [<VPIcon icon="fa-brands fa-firefox"/>ElementInternals API](https://developer.mozilla.org/en-US/docs/Web/API/ElementInternals).
...
![Diagram illustrating two steps of using formAssociated in a web development context. Step 1 explains registering the element with the browser, while Step 2 outlines attaching internal methods for form control behavior.](https://i0.wp.com/frontendmasters.com/blog/wp-content/uploads/2026/03/558964092-d9736006-52f1-4252-acca-39f9e71a56c4.png?resize=1024%2C484&ssl=1)
...
::: info Through `_internals`, you can:
...
The solution was a [<VPIcon icon="iconfont icon-lit"/>Lit Mixin](https://lit.dev/docs/composition/mixins/). It allows us to “plug in” form capabilities to any component while keeping the code DRY. To keep TypeScript happy with `protected` members, we use a companion `declare class`: this acts as a “blueprint” that tells the compiler exactly what the mixin is adding to the class.
...
![Diagram comparing two validation strategies in a web component context: 'Strategy 1: Delegate' showing how to manage validity with a host element and shadow DOM, and 'Strategy 2: Direct' illustrating direct validation handling without delegation.](https://i0.wp.com/frontendmasters.com/blog/wp-content/uploads/2026/03/558976906-b27cb5aa-c8a6-41d0-86f1-c29f06ec520f.png?resize=1024%2C381&ssl=1)
...
```ts
export function syncInnerInputValidity( internals: ElementInternals,
  inputEl:
    | HTMLInputElement
    | HTMLTextAreaElement
    | HTMLSelectElement
...
::: note The AgRadio Caveat

Even though AgRadio renders an inner `<input type="radio">`, delegation isn’t enough. Shadow DOM isolation actually breaks the native `required` constraint for radio groups. I’ll explain how to handle that specifically in the AgRadio section below.
...
If the component renders an inner native form control (like an `input`, `textarea`, or `select`), delegate. If it’s a custom-built widget like `AgToggle`, we own the validation logic.
...
The ultimate proof is in the [<VPIcon icon="fa-brands fa-firefox"/>FormData](https://developer.mozilla.org/en-US/docs/Web/API/FormData) object itself. Hook into a form’s submit event and log the `entries`:
...
::: note Wait, what if I don’t have a native input?

If you were building a custom slider or a star-rating component (Strategy 2), you wouldn’t “sync” from an inner element. Instead, you would manually call `this._internals.setValidity({ valueMissing: true }, "Message")` inside your own property setters (like `set value()`).
...
::: note The `value` property default

The component uses `this.value || 'on'` so that `FormData` always produces `'on'` when no explicit value is configured. The property itself defaults to `''`. This keeps form submission behavior correct while the property API stays clean.
...
Crucially, this isn’t “magic.” Because `checked` is a Lit `@property`, this manual assignment triggers the `updated()` [<VPIcon icon="iconfont icon-lit"/>lifecycle](https://lit.dev/docs/components/lifecycle/) on every radio in the group. We then **tap into that lifecycle** to run our glue code, explicitly calling `setFormValue()` and `_syncValidity()` to push the new state into the `ElementInternals` engine:
...
To understand why we need [<VPIcon icon="fa-brands fa-firefox"/>`this.getRootNode()`](https://developer.mozilla.org/en-US/docs/Web/API/Node/getRootNode), we have to look at where our `<ag-radio>` tags are actually being placed. It isn’t about the framework’s internal engine; it’s about whether the tags are sitting in the global document or inside a private “neighborhood”:
...
We have to manually verify the group state. Instead of asking the global `document`, we ask the element: “What is the root of the neighborhood I live in?” We use [<VPIcon icon="fa-brands fa-firefox"/>`this.getRootNode()`](https://developer.mozilla.org/en-US/docs/Web/API/Node/getRootNode) to find that root.
...
```component VPCard
{
  "title": "Form-Associated Custom Elements in Practice",
  "desc": "When you make a Web Component for a form element, you've got a bit of extra work to do to make sure they participate on the form in expected ways.",
  "link": "https://chanhi2000.github.io/bookshelf/frontendmasters.com/form-associated-custom-elements-in-practice.html",
  "logo": "https://frontendmasters.com/favicon.ico",
