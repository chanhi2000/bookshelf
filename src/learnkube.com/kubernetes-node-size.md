...
head:
  - - meta:
    ...
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/learnkube.com/kubernetes-node-size.html
...
<SiteInfo
  ...
  url="https://learnkube.com/kubernetes-node-size"
  logo="https://static.learnkube.com/f7e5160d4744cf05c46161170b5c11c9.svg"
  preview="https://static.learnkube.com/a102852d1e938e7c95a134501111ed92.png"/>
...
![Small vs. large nodes in a Kubernetes cluster](https://learnkube.com/a/c642b260295b87df85d97a6e8c20be48.svg)
...
![Resource allocations a in a Kubernetes node](https://learnkube.com/a/d627f4247a50662c83a2a40703a8b693.svg)
...
![Resource allocations a in a Kubernetes node with 2 vCPU and 8GB of memory](https://learnkube.com/a/3de0f4647a4b0d71b196d4e394aa5451.svg)
...
![Resource allocations a in a Kubernetes node with DaemonSets](https://learnkube.com/a/e77f8c687be1e38bd35470e177e4290a.svg)
...
![Resource allocations a in a Kubernetes node with 2 vCPU and 16GB of memory](https://learnkube.com/a/c72727b60ee6387d73c40004f9003561.svg)
...
![A node with 2 vCPU and 16GB of memory is insufficient to run seven replicas](https://learnkube.com/a/c1be9a8de0824f6e5abcba5371bb9b4e.svg)
...
![Resource allocations a in a Kubernetes node with 2 vCPU and 16GB of memory](https://learnkube.com/a/f97ca286843a3ff5ee0fbc9a55630829.svg)
...
![Comparing resource allocations between a cluster with a single node and another with multiple nodes](https://learnkube.com/a/23885fd9cefd09159d3bab9618a55aae.svg)
...
![Calculating the pod capacity for a 2 vCPU and 32GB worker node](https://learnkube.com/a/8166b554c895b638a68d2ef76475f943.svg)
...
![Calculating the pod capacity for a 1 vCPU and 4GB worker node](https://learnkube.com/a/2e379971f411f7ca2908ebed0798c020.svg)
...
![The replication factor for a cluster with two nodes and five replicas is two](https://learnkube.com/a/4db4fb1e1af84fcbcec51b23f9b9e77f.svg)
...
![The replication factor for a cluster with five nodes and five replicas is five](https://learnkube.com/a/4b0ddbe420f754323537c3bde7f938e2.svg)
...
You can scale applications deployed on Kubernetes [**using a combination of a horizontal scaler (i.e. increasing the number of replicas) and cluster autoscaler (i.e. increasing the nodes count)**](/learnkube.com/kubernetes-autoscaling-strategies.md).
...
![The Cluster Autoscaler provisions new nodes when pods are pending due to lack of resources.](https://learnkube.com/a/a26ee4d5052d327089d62b6da143f914.svg)

@tab 2/2

![When the node is provisioned, pods can be deployed.](https://learnkube.com/a/6a29222a3225ebb38f4e758210ca85b5.svg)
...
![Two clusters with: one pod per node and all pods into a single node](https://learnkube.com/a/f9fd8d7bfbc21dbf4e2f34d68489d1f7.svg)
...
![Pending pods triggering the autoscaler regardless of their size](https://learnkube.com/a/ab3adead1eea7f1615bff64488318317.svg)
...
![Autoscaling increments in large and small nodes](https://learnkube.com/a/4b5b12047d321e34a6c2455677fd99a0.svg)
...
![Trade-offs for autoscaling increments in Kubernetes nodes](https://learnkube.com/a/be78be3cd35dc7387e2352c4630c5ae9.svg)
...
- [**Invoke the Container Runtime Interface (CRI) to create the Pod sandbox. The CRI invokes the Container Network Interface (CNI) to attach the Pod to the network**](/learnkube.com/kubernetes-network-packets#how-linux-network-namespaces-work-in-a-pod.md).
...
![The Kubelet and the CRI, CSI and CNI interfaces](https://learnkube.com/a/b321498cc64bfd4bc424abd7a8a8f461.svg)
...
![The container runtime download the container image once and runs 13 replicas](https://learnkube.com/a/352ea906f7800c90ed71deea01eac4f1.svg)
...
![Each of the 13 container runtimes download one image](https://learnkube.com/a/d4dfe6e68938734993dd6b5bd2498e7b.svg)
...
![Imagine having a cluster with 8 nodes, one replica per node.](https://learnkube.com/a/c77ad0aa9e0afa24e57788948ee1b12c.svg)

@tab 2/4

![The cluster is full; scaling to 16 replicas triggers the cluster autoscaler.](https://learnkube.com/a/d3b49aee1e2a56dfe0509c3874c308a8.svg)

@tab 3/4

![As soon as the nodes are provisioned, the Container Runtime downloads the container image.](https://learnkube.com/a/6b09aa74b75b631b7be55c492d80c084.svg)

@tab 4/4

![Finally, the pods are created in the nodes.](https://learnkube.com/a/4f053af905c5ceb1f89c9a70dcabebd9.svg)
...
![A single kubelet issueing 5 requests per second](https://learnkube.com/a/cb5baa1a4b290bff10e3f693fb5fce97.svg)

The second 65 requests per second (i.e. `13 x 5`).

![13 kubelets issueing 5 requests per second each](https://learnkube.com/a/106f96256da416271d46d757e912c983.svg)
...
![Each worker node has a subnet assigned](https://learnkube.com/a/cc3274f66d411c87546f1eb88a42511d.svg)
...
![When a Pod is deleted, the kubelet is notified of the change.](https://learnkube.com/a/01b4a22bf8aa92a7998540f6ac1516e0.svg)

@tab 2/4

![If the Pod has a preStop hook, it is invoked first. Then, the kubelet sends the SIGTERM signal to the container.](https://learnkube.com/a/ba13194ea5b9ef63597e8fb059355887.svg)

@tab 3/4

![By default, the process has 30 seconds to exit, including the preStop hook. If the process isn't exited by then, the kubelet sends the SIGKILL signal and forces killing the process.](https://learnkube.com/a/eaba36dda660dd289189979c3b6ce3c4.svg)

@tab 4/4

![The kubelet notifies the control plane that the Pod was deleted successfully. The IP address is finally released.](https://learnkube.com/a/c05c50cd03dba4465b38b95a016b3712.svg)
...
![Imagine your node is using all available IP addresses.](https://learnkube.com/a/e038d5441bbab5e27bff91fa567517e0.svg)

@tab 2/3

![When a pod is deleted, the IP address is not released immediately. You have to wait for the graceful shutdown.](https://learnkube.com/a/f0bdd2ddeb962d9e3303965eef0006d3.svg)

@tab 3/3

![As soon as the pod is deleted, the IP address can be reused.](https://learnkube.com/a/362ab748b014147011e2a723ae8fa71b.svg)
...
![The ingress controller routes traffic to an IP address.](https://learnkube.com/a/7b2d466a8559f46f477801ae2ca257fd.svg)

@tab 2/2

![If the IP address is recycled and used by a new Pod without waiting for the graceful shutdown, the ingress controller might still route traffic to that IP address.](https://learnkube.com/a/a785b7150b052e5f5f6906c17c7312b8.svg)
...
[<VPIcon icon="fas fa-globe"/>The Kubernetes instance calculator](https://learnkube.com/kubernetes-instance-calculator/) lets you explore the best instance type for a given workload.
...
```component VPCard
{
  "title": "Architecting Kubernetes clusters — choosing a worker node size",
  "desc": "What type of worker nodes should I use for my Kubernetes cluster? And how many of them?. This article looks at the pros and cons.",
  "link": "https://chanhi2000.github.io/bookshelf/learnkube.com/kubernetes-node-size.html",
  "logo": "https://static.learnkube.com/f7e5160d4744cf05c46161170b5c11c9.svg",
