---
lang: en-US
title: "How To Create a Multi-Node MySQL Cluster on Ubuntu 18.04"
description: "Article(s) > How To Create a Multi-Node MySQL Cluster on Ubuntu 18.04"
...
head:
  - - meta:
    - property: og:title
      content: "Article(s) > How To Create a Multi-Node MySQL Cluster on Ubuntu 18.04"
    - property: og:description
      content: "How To Create a Multi-Node MySQL Cluster on Ubuntu 18.04"
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/digitalocean.com/how-to-create-a-multi-node-mysql-cluster-on-ubuntu-18-04.html
...
cover: https://community-cdn-digitalocean-com.global.ssl.fastly.net/st7M6NJRZgh4AfdzwcjP94uq
---
...
<SiteInfo
  name="How To Create a Multi-Node MySQL Cluster on Ubuntu 18.04"
  desc="The MySQL Cluster distributed database provides high availability and throughput for your MySQL database management system. This tutorial demonstrates how to… "
  url="https://digitalocean.com/community/tutorials/how-to-create-a-multi-node-mysql-cluster-on-ubuntu-18-04"
  logo="https://digitalocean.com/_next/static/media/favicon.594d6067.ico"
  preview="https://community-cdn-digitalocean-com.global.ssl.fastly.net/st7M6NJRZgh4AfdzwcjP94uq"/>
...
- Three Ubuntu 18.04 Droplets with [<VPIcon icon="fa-brands fa-digital-ocean"/>private networking](https://digitalocean.com/docs/networking/private-networking/quickstart) enabled
- A non-root user with sudo privileges configured for each Droplet. You can learn how to do this in [**Initial Server Setup with Ubuntu 18.04**](/digitalocean.com/initial-server-setup-with-ubuntu-18-04.md).
...
To install the Cluster Manager, we first need to fetch the appropriate `.deb` installer file from the the official MySQL Cluster [<VPIcon icon="iconfont icon-mysql"/>download page](http://dev.mysql.com/downloads/cluster/).
...
The Cluster Manager should be the first component launched in any MySQL cluster. It requires a configuration file, passed in as an argument to its executable. We’ll create and use the following configuration file: <VPIcon icon="fas fa-folder-open"/>`/var/lib/mysql-cluster/``config.ini`.

On the Cluster Manager Droplet, create the <VPIcon icon="fas fa-folder-open"/>`/var/lib/mysql-cluster` directory where this file will reside:
...
```ini title="/var/lib/mysql-cluster/config.ini"
[ndbd default]
...
[mysqld]
# SQL node options:
hostname=198.51.100.2 # In our case the MySQL server/client is on the same Droplet as the cluster manage
```
...
# 2018-07-25 21:48:39 [MgmtSrvr] INFO     -- Successfully created config director
```
...
```ini title="/etc/systemd/system/ndb_mgmd.service"
[Unit]
...
Here, we’ve added a minimal set of options instructing systemd on how to start, stop and restart the `ndb_mgmd` process. To learn more about the options used in this unit configuration, consult the systemd [<VPIcon icon="fas fa-globe"/>manual](https://freedesktop.org/software/systemd/man/systemd.service.html).
...
#            └─11193 /usr/sbin/ndb_mgmd -f /var/lib/mysql-cluster/config.in
```
...
We’ll add rules to allow local incoming connections from both data nodes. After entering these commands, you should see the following output:
...
To install the data node binaries we first need to fetch the appropriate `.deb` installer file from the official MySQL [<VPIcon icon="iconfont icon-mysql"/>download page](http://dev.mysql.com/downloads/cluster/).
...
The data nodes pull their configuration from MySQL’s standard location, <VPIcon icon="fas fa-folder-open"/>`/etc/`<VPIcon icon="fas fa-gears"/>`my.cnf`. Create this file using your favorite text editor and begin editing it:
...
ndb-connectstring=198.51.100.2  # location of cluster manage
```
...
In our example, the data node will find out that its data directory is <VPIcon icon="fas fa-folder-open"/>`/usr/local/mysql/`<VPIcon icon="fas fa-database"/>`data`, per the manager’s configuration. Before starting the daemon, we’ll create this directory on the node:

```sh
sudo mkdir -p /usr/local/mysql/data
```

Now we can start the data node using the following command. You should see the following output:

```sh
sudo ndbd
#
# 2018-07-18 19:48:21 [ndbd] INFO     -- Angel connected to '198.51.100.2:1186'
2018-07-18 19:48:21 [ndbd] INFO     -- Angel allocated nodeid: 
```
...
We’ll add rules to allow incoming connections from the Cluster Manager and other data nodes. After entering these commands, you should see the following output:
...
```ini title="/etc/systemd/system/ndbd.service"
[Unit]
...
Here, we’ve added a minimal set of options instructing systemd on how to start, stop and restart the `ndbd` process. To learn more about the options used in this unit configuration, consult the systemd [<VPIcon icon="fas fa-globe"/>manual](https://freedesktop.org/software/systemd/man/systemd.service.html).
...
You can verify that the NDB Cluster Management service is running. You should see the following output:
...
We’ll once again grab the MySQL Cluster Server binary from the official MySQL Cluster [<VPIcon icon="iconfont icon-mysql"/>download page](http://dev.mysql.com/downloads/cluster/).

From this page, under **Select Operating System**, choose **Ubuntu Linux**. Then, under **Select OS Version**, choose **Ubuntu Linux 18.04 (x86, 64-bit)**.

Scroll down until you see **DEB Bundle**, and click on the **Download** link (it should be the first one in the list). You will be brought to a **Begin Your Download** page. Here, right click on **No thanks, just start my download.** and copy the link to the <VPIcon icon="fas fa-file-zipper"/>`.tar` archive.

Now, log in to the Cluster Manager Droplet (in this tutorial, `198.51.100.2`), and download this <VPIcon icon="fas fa-file-zipper"/>`.tar` archive (recall that we are installing MySQL Server on the same node as our Cluster Manager – in a production setting you should run these daemons on different nodes):
...
The configuration for MySQL Server is stored in the default <VPIcon icon="fas fa-folder-open"/>`/etc/mysql/`<VPIcon icon="fas fa-gears"/>`my.cnf` file.
...
```ini :collapsed-lines title="/etc/mysql/my.cnf"
...
[mysql_cluster]
# Options for NDB Cluster processes:
ndb-connectstring=198.51.100.2  # location of management serve
```
...
Enter your password when prompted, and hit <kbd>ENTER</kbd>.
...
To exit the MySQL prompt, simply type `quit` or press <kbd>CTRL</kbd>+<kbd>D</kbd>.
...
Open the Cluster management console, `ndb_mgm` using the command. You should see the following output:
...
Once inside the console enter the command `SHOW` and hit <kbd>ENTER</kbd>. You should see the following output:
...
The above shows that there are two data nodes connected with `node-id`s 2 and 3. There is also one management node with `node-id` 1 and one MySQL server with `node-id` 4. You can display more information about each id by typing its number with the command `STATUS` as follows. The above command shows you the status, MySQL version, and NDB version of node 2:
...
To exit the management console type `quit`, and then hit <kbd>ENTER</kbd>.

The management console is very powerful and gives you many other options for administering the cluster and its data, including creating an online backup. For more information consult the [<VPIcon icon="iconfont icon-mysql"/>official MySQL documentation](https://dev.mysql.com/doc/refman/5.7/en/mysql-cluster-management.html).
...
You can also set the default storage engine to `ndbcluster` in the `my.cnf` file that we edited previously. If you do this, you won’t need to specify the `ENGINE` option when creating tables. To learn more, consult the MySQL [<VPIcon icon="iconfont icon-mysql"/>Reference Manual](https://dev.mysql.com/doc/refman/5.7/en/storage-engine-setting.html).

---

## Conclusion

In this tutorial, we’ve demonstrated how to set up and configure a MySQL Cluster on Ubuntu 18.04 servers. It’s important to note that this is a minimal, pared-down architecture used to demonstrate the installation procedure, and there are many advanced options and features worth learning about before deploying MySQL Cluster in production (for example, performing backups). To learn more, consult the official [<VPIcon icon="iconfont icon-mysql"/>MySQL Cluster  documentation](https://dev.mysql.com/doc/refman/5.7/en/mysql-cluster.html).
...
```component VPCard
{
  "title": "How To Create a Multi-Node MySQL Cluster on Ubuntu 18.04",
  "desc": "The MySQL Cluster distributed database provides high availability and throughput for your MySQL database management system. This tutorial demonstrates how to… ",
  "link": "https://chanhi2000.github.io/bookshelf/digitalocean.com/how-to-create-a-multi-node-mysql-cluster-on-ubuntu-18-04.html",
  "logo": "https://digitalocean.com/_next/static/media/favicon.594d6067.ico",
