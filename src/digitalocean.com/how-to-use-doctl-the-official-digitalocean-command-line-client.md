...
head:
  - - meta:
    - property: og:title
      content: "Article(s) > How To Use Doctl, the Official DigitalOcean Command-Line Client"
    - property: og:description
      content: "How To Use Doctl, the Official DigitalOcean Command-Line Client"
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/digitalocean.com/how-to-use-doctl-the-official-digitalocean-command-line-client.html
prev: /devops/digitalocean/articles/README.md
date: 2016-03-25
isOriginal: false
author: 
  - name: Marko Mudrinić
    url: https://digitalocean.com/community/users/xmudrii
  - name: Hazel Virdó
    url: https://digitalocean.com/community/users/hazelnym
cover: https://community-cdn-digitalocean-com.global.ssl.fastly.net/jZSgSpiNS3CG5J25sfCSTdxW
---
...
<SiteInfo
  name="How To Use Doctl, the Official DigitalOcean Command-Line Client"
  desc="DigitalOcean’s web based control panel provides a convenient, point-and-click interface for managing Droplets. There are many times, however, when a command-… "
  url="https://digitalocean.com/community/tutorials/how-to-use-doctl-the-official-digitalocean-command-line-client"
  logo="https://digitalocean.com/_next/static/media/favicon.594d6067.ico"
  preview="https://community-cdn-digitalocean-com.global.ssl.fastly.net/jZSgSpiNS3CG5J25sfCSTdxW"/>
...
The `doctl compute droplet get` command supports output templating, which lets you customize the format of the output. To use this feature, specify [<VPIcon icon="fa-brands fa-golang"/>the Go-formatted template](https://golang.org/pkg/text/template/) via the `--template` flag.
...
```sh
doctl compute droplet list  # Your Droplets. Some commands also take the name;
                            # most require the numeric value from the **ID** column.

doctl compute ssh-key list  # The SSH keys associated with your account.
                            # For Droplet creation, you can specify either the 
                            # **numeric ID** or **fingerprint**.

doctl compute region list   # Available regions. Use the string in the **Slug** column.

doctl compute image list    # Available images, including snapshots, backups, and base distribution images.
                            # Use the string in the **Slug** column for creating new Droplets.
...
```sh
doctl compute droplet actions <DROPLET_ID>  #  Display a history of actions taken for a Droplet.

doctl compute droplet backups <DROPLET_ID>  # List backups for a Droplet.

doctl compute droplet create name \.        # Create a Droplet. Size, image and region are all mandatory.
--size s-1vcpu-1gb \
--image image_slug \
--region nyc1                               


doctl compute droplet delete <DROPLET_ID_OR_NAME>   # Delete a Droplet by id or name.

doctl compute droplet get <DROPLET_ID>.             # Get details for a particular Droplet

doctl compute droplet kernels <DROPLET_ID>          # List kernels for a Droplet. 

doctl compute droplet list                          # List your current Droplets.

doctl compute droplet neighbors <DROPLET_ID>        # List your Droplets running on the same physical hardware
                                                    # as a specific Droplet.
doctl compute droplet snapshots <DROPLET_ID         # List snapshots for a Droplet.

doctl compute droplet tag <DROPLET_ID/DROPLET_NAME> # Tag a Droplet.

doctl compute droplet untag <DROPLET_ID/DROPLET_NAME> # Untag a Droplet.
```
...
| `enable-private-networking droplet_id` | Enable [**private networking**](/digitalocean.com/how-to-set-up-and-use-digitalocean-private-networking.md) for a Droplet. |
...
SSH command forwarding is currently not available on Windows.
...
A Floating IP is a publicly-accessible static IP address that can be assigned to one of your Droplets. For a detailed description of the feature, you can read [**How To Use Floating IPs on DigitalOcean**](/digitalocean.com/how-to-use-floating-ips-on-digitalocean.md). You can manipulate floating IPs with `doctl compute floating-ip`.
...
The `doctl compute volume` command can be used to create, delete, or get information about DigitalOcean’s Block Storage volumes. For more information about this feature, read our guide on [**How To Use Block Storage on DigitalOcean**](/digitalocean.com/how-to-use-block-storage-on-digitalocean.md).
...
The `doctl compute load-balancer` command can be used to create, delete, or get information about DigitalOcean’s Load Balancers. For more information about this feature, read our [**Introduction to DigitalOcean Load Balancers**](/digitalocean.com/an-introduction-to-digitalocean-load-balancers.md).
...
The `doctl compute firewall` command lets you create and manage Firewalls, including creating and maintaining rules. For more about information about administering Firewalls using `doctl`, check out the [**How To Secure Web Server Infrastructure With DigitalOcean Cloud Firewalls Using Doctl**](/digitalocean.com/how-to-secure-web-server-infrastructure-with-digitalocean-cloud-firewalls-using-doctl.md) tutorial.
...
Tags are used to apply custom labels to resources, allowing you to easily filter them. You can learn more about Tags in [**the How To Tag DigitalOcean Droplets tutorial**](/digitalocean.com/how-to-tag-digitalocean-droplets.md).
...
```component VPCard
{
  "title": "How To Use Doctl, the Official DigitalOcean Command-Line Client",
  "desc": "DigitalOcean’s web based control panel provides a convenient, point-and-click interface for managing Droplets. There are many times, however, when a command-… ",
  "link": "https://chanhi2000.github.io/bookshelf/digitalocean.com/how-to-use-doctl-the-official-digitalocean-command-line-client.html",
  "logo": "https://digitalocean.com/_next/static/media/favicon.594d6067.ico",
