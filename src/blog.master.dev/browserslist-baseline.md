...
head:
  - - meta:
    ...
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/blog.master.dev/browserslist-baseline.html
...
cover: https://blog.master.dev/wp-json/social-image-generator/v1/image/7741
---
...
<SiteInfo
  ...
  url="https://blog.master.dev/browserslist-baseline/"
  logo="https://blog.master.dev/favicon.ico"
  preview="https://blog.master.dev/wp-json/social-image-generator/v1/image/7741"/>

I saw [Tony Conway (<VPIcon icon="iconfont icon-github"/>`tonypconway`)](https://github.com/tonypconway) & Jeremy Wagner’s post on web.dev, [**Use Baseline with Browserslist**](/web.dev/use-baseline-with-browserslist.md), and I had a little play with it myself ([<VPIcon icon="fa-brands fa-youtube"/>saved live stream](https://youtube.com/live/PAT1cCA9kYw)). Allow me to write down what I know and what I learned.

So here’s [<VPIcon icon="fas fa-globe"/>Browserslist](https://browsersl.ist/)[^1]
...
![The `oklch()` color is left alone, but we still see the `light-dark()` transformation and the vendor prefixing for `mask`.](https://i0.wp.com/master.dev/blog/wp-content/uploads/2025/11/CleanShot-2025-11-12-at-15.29.33%402x.png?resize=1024%2C545&ssl=1)
...
![Just some trivial conversion of colors, which is mostly a minification thing.](https://i0.wp.com/master.dev/blog/wp-content/uploads/2025/11/CleanShot-2025-11-12-at-15.32.47%402x.png?resize=1024%2C361&ssl=1)
...
Remember that CSS features aren’t always transformable to backwards-compatible things. Like if you use `@layer` or `rlh` units or something, that’s just going to get left alone despite any browser support levels. Same with stuff like `shape()`, which [**is brand new**](/blog.master.dev/shape-a-new-powerful-drawing-syntax-in-css.md), and would be awesome if they ported it to something, but alas, they do not.
...
The *best* possible move is to use your own analytics data to inform the choices. This can be done with a Baseline Target Report and Jeremy Wagner and Rachel Andrew get into it in [**How to choose your Baseline target**](/web.dev/how-to-choose-your-baseline-target.md).
...
```component VPCard
{
  "title": "Browserslist & Baseline",
  "desc": "I saw Tony Conway & Jeremy Wagner’s post on web.dev, Use Baseline with Browserslist, and I had a little play with it myself (saved live stream). Allow me to write down what I know and what I learned. So here’s Browserslist. Browserslist is the developer community at it’s best. There are a bunch of tools […]",
  "link": "https://chanhi2000.github.io/bookshelf/blog.master.dev/browserslist-baseline.html",
  "logo": "https://blog.master.dev/favicon.ico",
