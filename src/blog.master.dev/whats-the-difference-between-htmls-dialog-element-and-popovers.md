...
head:
  - - meta:
    ...
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/blog.master.dev/whats-the-difference-between-htmls-dialog-element-and-popovers.html
...
cover: https://blog.master.dev/wp-json/social-image-generator/v1/image/4069
---
...
<SiteInfo
  ...
  preview="https://blog.master.dev/wp-json/social-image-generator/v1/image/4069"/>
...
They are essentially `position: fixed;` and `margin: auto;` which centers them in the viewport. This is a probably a smart default for dialogs. In my opinion, popovers are usually begging for [<VPIcon icon="fa-brands fa-firefox" />anchor positioning](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_anchor_positioning) to open the popover near where the element that opened it is, but they [**work nicely as slide-out drawers**](/blog.master.dev/popovers-work-pretty-nicely-as-slide-out-drawers.md) as well, particularly on mobile.
...
Both dialogs and popovers can have multiple open at once. The most recent one to be opened will be the one that is most “on top” and will close the first via soft dismiss or the ESC key. (Also [**see the CloseWatcher API**](/blog.master.dev/closewatcher.md)).
...
Popovers likely have more use cases than dialogs. Any time you [**need a tooltip**](/blog.master.dev/using-the-popover-api-for-html-tooltips.md) or to provide more contextual information that has good reason not to be visible by default, a popover is a good choice.
...
This is all very cutting edge right now, so browser support is spotty, but both of these elements [**can be animated both on the way in and out**](/blog.master.dev/the-dialog-element-with-entry-and-exit-animations.md).
...
```component VPCard
{
  "title": "What’s the Difference Between HTML’s Dialog Element and Popovers?",
  "desc": "They are pretty similar in both look and functionality, but are have some important differences, slightly different APIs, and functionality. The use cases are also a bit different, so let's have a look!",
  "link": "https://chanhi2000.github.io/bookshelf/blog.master.dev/whats-the-difference-between-htmls-dialog-element-and-popovers.html",
  "logo": "https://blog.master.dev/favicon.ico",
