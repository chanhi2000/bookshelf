...
description: "Article(s) > Two Week Build"
...
head:
  - - meta:
    - property: og:title
      content: "Article(s) > Two Week Build"
    - property: og:description
      content: "Two Week Build"
    - property: og:url
      content: https://chanhi2000.github.io/bookshelf/dbushell.com/two-week-build.html
...
<SiteInfo
  name="Two Week Build"
  desc="Two Week Build"
  url="https://dbushell.com/2014/04/24/two-week-build/"
...
From concept to launch the [**new version**](/dbushell.com/spring-refresh.md) of my website clocked just over two weeks of development time. This article documents assorted aspects of that process.

---

## Initial Concept

![dbushell.com design in Photoshop](https://dbushell.com/images/blog/2014/dbushell-photoshop.png)
...
I gave myself a head start using a stripped back version of my [**Tales theme**](/dbushell.com/introducing-tales.md) (which itself is based of [**Origin**](/dbushell.com/origin.md)). A quick export / import into a local WordPress install gave me a platform with real content to build and design upon.
...
Inspired by Marcin Wichary’s work at [<VPIcon icon="fa-brands fa-medium"/>Medium](https://medium.design/crafting-link-underlines-on-medium-7c03a9274f9), I’m using `linear-gradient` to give fine control over hyperlink underlines. Combined with `text-shadow` to clear descenders this is a very pretty technique:

![CSS link styles on dbushell.com](https://dbushell.com/images/blog/2014/dbushell-link-style.png)
...
**Droid Serif** is a personal favourite. The font is free to self-host under the Apache License but I’m using [<VPIcon icon="fas fa-globe"/>Typekit](https://typekit.com/) for ease-of-use (and to justify my subscription). Fonts are loaded asynchronously because I’d rather have a flash of content than none at all.

---

## Performance Budget

Speaking of web fonts, they account for almost 50% of my [<VPIcon icon="fas fa-globe"/>home page](https://dbushell.com/) download. Sounds excessive, but at a grand total of **248kb** my website is positively slim. Given the design’s emphasis on typography I find these bytes are well spent.

A run through [<VPIcon icon="fas fa-globe"/>WebPageTest](https://webpagetest.org/result/140422_NR_MTQ/) gives strong local results:

![dbushell.com performance test](https://dbushell.com/images/blog/2014/dbushell-webpagetest.png)

Aside from [**Varnish Cache**](/dbushell.com/performance-varnish-cache-wordpress.md) on my server I’m not doing anything special. Typekit and Google Analytics’ expire headers are giving me a “C” at the end which isn’t too worrying. For a handful of small resources I’m not sure using a CDN[^1] is worth the investment, or maybe it is… something I need to explore.
...
I’ve built many websites with font icons, many others with SVG sprites, but I’m now a full convert to inline SVG using the `<use>` technique (see [<VPIcon icon="fas fa-globe"/>Codrops](http://tympanus.net/codrops/2013/11/27/svg-icons-ftw/) and [**CSS-Tricks**](/css-tricks.com/svg-sprites-use-better-icon-fonts.md) for details). At least for icons that are used globally.
...
With feature detection and [**automation**](/dbushell.com/updates-to-origin.md) it takes little effort to provide a fallback PNG.
...
```component VPCard
{
  "title": "Two Week Build",
  "desc": "Two Week Build",
  "link": "https://chanhi2000.github.io/bookshelf/dbushell.com/two-week-build.html",
  "logo": "https://dbushell.com/assets/icons/favicon.svg",
